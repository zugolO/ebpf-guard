...
static __always_inline void fill_tls_process_info(struct tls_event *e)
{
	 ...
	 * компилировалась, но ни разу не подавалась ядру — смок 24.09.2026 на
	 * ebaka2 предъявил её первым же живым запуском. Идиома взята у соседей,
	 * которые на этом ядре грузятся: tls_clienthello.bpf.c:85,
