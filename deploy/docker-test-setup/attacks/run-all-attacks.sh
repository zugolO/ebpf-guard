...
run_host_block_device_positive_control() {
    # 6.0j/п.5 (находка №211). Почему контроль стоит ЗДЕСЬ, в окне атак, а не
    # только шагом 6.0l пайплайна: состав детекта (крит. 6) и «немые правила»
    # (крит. 5.9.4h) гейт считает по final-metrics-$TIMESTAMP.txt и
    # final-alerts-$TIMESTAMP.json — снимкам, которые get_final_metrics
    # снимает в КОНЦЕ этого скрипта. Все шаги-контроли пайплайна (6.0c…6.0l)
    # исполняются ПОСЛЕ него (находка №206: контроли легли за правой границей
    # окна), поэтому их алерты в эти снимки не попадают вовсе. Контроль,
    # стоящий только на шаге 6.0l, доказал бы правило живым и всё равно
    # оставил бы гейту оба FAIL замера №6.0f2 — «потеряно 1 типов детекта» и
    # «1 правило немо без строки в silent-rules.txt».
    #
    # Разделение труда с 6.0.18 (шаг 6.0l): здесь — ВХОД в снимок прогона
    # (правило перестаёт быть немым для гейта), там — ВЕРДИКТ (>= 1 warning
    # при 0 критикалов по обоим id разведения №200), со своим die.
    #
    # Примитив безвреден по построению: dumpe2fs -h открывает устройство
    # только на чтение суперблока — тот же вызов, который негативный контроль
    # 6.0.13 использует как образец ШТАТНОЙ активности хоста.
    log "==========================================="
    log "ПОЗИТИВНЫЙ КОНТРОЛЬ 6.0j/п.5 (находка №211): dumpe2fs -h по raw-устройству хоста"
    log "==========================================="

    local marker="$RESULTS_DIR/host-block-device-control-$TIMESTAMP.txt"
    local dev=""
    local cand
    for cand in /dev/vda1 /dev/sda1 /dev/vda /dev/sda; do
        if [ -b "$cand" ]; then dev="$cand"; break; fi
    done
    if [ -z "$dev" ] || ! command -v dumpe2fs &> /dev/null; then
        warn "нет блочного устройства из списка или нет dumpe2fs(8) — позитивный контроль №211 пропущен, container_escape_host_device_from_host останется немым для гейта (оба FAIL замера №6.0f2 воспроизведутся)"
        {
            echo "skipped=1"
            echo "skip_reason=устройство='${dev:-нет}' dumpe2fs=$(command -v dumpe2fs 2>/dev/null || echo нет)"
        } > "$marker"
        echo ""
        return
    fi

    local hd_done=0 hd_rc=0
    mark_attack_window
    # `|| hd_rc=$?`, а не голый вызов с чтением $? следующей строкой: в этом
    # файле стоит `set -e` (строка 5), и ненулевой dumpe2fs оборвал бы ВЕСЬ
    # прогон атак посреди окна — цена ошибки здесь не «контроль не исполнён»,
    # а «замер потерян», тот же класс, что находки №176/№208/№209.
    dumpe2fs -h "$dev" >/dev/null 2>&1 || hd_rc=$?
    # Сторож по результату (память «позитивный контроль по результату»):
    # rc != 0 означает, что устройство не читалось вовсе — не ext-раздел, нет
    # прав, устройство занято, — и молчание правила тогда приборное, а не
    # продуктовое. Различие записывается в маркер, а не домысливается потом.
    if [ "$hd_rc" -eq 0 ]; then
        hd_done=1
        log "dumpe2fs -h $dev выполнен (rc=0), comm=dumpe2fs, контейнерного обогащения на событии нет — ожидается срабатывание container_escape_host_device_from_host (warning) и НИ ОДНОГО критикала container_escape_host_device"
    else
        warn "dumpe2fs -h $dev вернул rc=$hd_rc — позитивный контроль №211 не исполнен, ноль алертов правила будет приборным"
    fi
    mark_attack_window

    {
        ...
        echo "comm=dumpe2fs"
        echo "device=$dev"
        echo "rule_id=container_escape_host_device_from_host"
    } > "$marker"

    # Запись в манифест НЕ делается сознательно: манифест — знаменатель
    # крит. 7 (recall по категориям), и новая категория без своей строки
    # сопоставления в гейте уронила бы recall 13/13 на шаге, который к recall
    # отношения не имеет. Вход этого контроля в гейт идёт через состав
    # детекта, а не через манифест.
    echo ""
}
