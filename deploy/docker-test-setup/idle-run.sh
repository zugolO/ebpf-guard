...
# 5.9.8g (находка №95): регистрация здесь — $$ этого скрипта, НЕ сабшелла —
# и это осознанно остаётся так же и для постобработки (P0-3 рестарт,
# journalctl, сборка SUMMARY/tar, всё после конца измерительного цикла
# DURATION и до строки "IDLE RUN DONE" ниже). В отличие от run-all-attacks.sh
# (5.9.7g, находка №86), где сабшелл сузил регистрацию до пролога именно
# потому, что весь остальной скрипт порождает АТАКУЮЩИЙ трафик, который
# нельзя прятать от корреляции, — idle-run.sh не порождает ничего, кроме
# собственного шума измерителя, ни на одном этапе своей жизни. Сужать
# регистрацию до подскрипта здесь означало бы открыть ровно ту дыру,
# которую эта волна закрывает: постобработка (systemctl restart, tar,
# journalctl — уже перечислены комментарием выше) осталась бы видимой
# корреляции и наводила бы алерты в точности в той точке, где начинается
# слепое окно критерия 16 (run-gate.sh, секция 16, харнесс-список 5.9.8g).
# trap ниже освобождает корень ТОЛЬКО при выходе процесса — то есть после
# постобработки целиком, а не после DURATION-цикла.
