...
# РИСК №3 постановки: сужение двух правил по ssh (5.9.7e) обязано иметь живой
# позитивный контроль на ТОМ ЖЕ прогоне — юнит-теста мало, он не проверяет ни
# ядро, ни конвейер. Без шага в пайплайне секция гейта «5.9.7e: позитивный
# контроль» валит замер по нулю срабатываний, и это правильно; здесь оно
# ловится за минуты, а не через полтора часа.
echo "--- преflight: позитивный контроль 5.9.7e присутствует в цепочке (риск №3) ---"
grep -q 'run_ssh_keys_positive_control()' $SETUP/attacks/run-all-attacks.sh     || die "риск №3: run_ssh_keys_positive_control отсутствует — сужение rootkit_ssh_authorized_keys_modified нечем доказать живьём"
grep -qE '^[[:space:]]+run_ssh_keys_positive_control[[:space:]]*$' $SETUP/attacks/run-all-attacks.sh     || die "риск №3: run_ssh_keys_positive_control определён, но не вызывается — шаг не исполнится"
