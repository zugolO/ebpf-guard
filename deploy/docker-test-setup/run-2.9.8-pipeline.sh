#!/usr/bin/env bash
# ЗАМЕР №2.9.8 — приёмка волны 5.9.8, фактический вход в волну 6.
#
# Структура унаследована от №2.9.7 (преflight -> реплеи -> сборка -> P0-3 ->
# очистка -> SMOKE -> контроли вне окна -> пауза -> idle-час -> атаки -> гейт
# -> отчёт, ОДНИМ detached-процессом). Отличия от №2.9.7 — пять, и каждое
# названо постановкой №2.9.8, а не добавлено по вкусу:
#
#   [1/14] ЖЁСТКИЙ СТОП №1 расширен ТРЕТЬИМ архивом — collect-2.9.7.
#          replay-gate.sh теперь делает четыре реплея, и четвёртый обязан
#          дать крит. 9 = 88.6/мин, а не SKIP (5.9.8e, №90). Реплей, который
#          не воспроизводит уже известный ответ, — стоп: это единственная
#          защита от риска №1 («дефекты измерителя чинятся тем же кодом,
#          который их меряет»), пятую волну подряд.
#
#   [3/14] make generate НЕ ПРОСТО обязателен — он обязателен ПОД ПРОВЕРКОЙ.
#          5.9.8a правит bpf/dns.bpf.c (ключ dns_socket_map: pid_tgid → tgid).
#          Прогон на старых заглушках *_bpf_gen.go измерил бы не тот объект,
#          который собран ([[bpf-gen-files-are-stubs]]), и дал бы зелёный
#          крит. 15 при неправленом ядре.
#
#   [7/14] ЖЁСТКИЙ СТОП №4 (НОВЫЙ) — оба контроля DNS до окна замера.
#          Запрет №6 постановки: правка dns_socket_map не принимается без
#          негативного (ClientHello по переиспользованному fd не даёт
#          DNS-события) И позитивного (межпоточный резолв даёт события)
#          контроля на одном прогоне. Риск №2 постановки: ошибка в
#          bpf/dns.bpf.c даёт «0 long-label за idle-час» одинаково и при
#          починке, и при полностью ослепшем DNS-коллекторе. Слепой
#          коллектор на idle-час не едет.
#
#   [6/14] SMOKE-гейт дополнен двумя живыми признаками волны: серия
#          ebpf_guard_counting_canary_total присутствует в /metrics (без неё
#          крит. 20 молча свалится в устаревший путь с вычетом фона, 5.9.8b)
#          и reason="bad_header" материализован в dns_decode_errors_total
#          (5.9.8a). Обе — материализованные нулём при старте, поэтому их
#          отсутствие означает именно старый бинарь, а не «ещё не случилось».
#
# Запуск (обязательно detached, иначе обрыв ssh убивает замер):
#   setsid nohup bash /opt/ebpf-guard/deploy/docker-test-setup/run-2.9.8-pipeline.sh >/dev/null 2>&1 &
# Готовность: файл /root/PIPELINE-2.9.8-DONE. Полный лог: /root/run-2.9.8-pipeline.log
#
...
exec > /root/run-2.9.8-pipeline.log 2>&1
set -x
SETUP=/opt/ebpf-guard/deploy/docker-test-setup
IDLE_OUT=$SETUP/idle-results/idle-2.9.8
P0_OUT=$SETUP/idle-results/p0-3-2.9.8
export PATH="$PATH:/usr/local/go/bin"

die() {
    echo "=== СТОП: $* ==="
    echo "=== ЗАМЕР №2.9.8 НЕ НАЧАТ. $(date -u +%H:%M:%S) UTC ==="
    touch /root/PIPELINE-2.9.8-DONE
    exit 1
}
...
echo "=== [1/14] ЖЁСТКИЙ СТОП №1: replay-gate.sh на ТРЁХ архивах (5.9.7c/5.9.8e) ==="
...
# падать; collect-2.9.7 (НОВОЕ, 5.9.8e) — крит. 9 даёт 88.6/мин, а не SKIP.
find_archive() {
    local name="$1" d
...
if [ -z "$C295_DIR" ] || [ -z "$C296_DIR" ] || [ -z "$C297_DIR" ]; then
    die "архивы collect-2.9.5/2.9.6/2.9.7 не найдены на стенде. Скопировать (например в /root/) либо задать REPLAY_C295_DIR/REPLAY_C296_DIR/REPLAY_C297_DIR. Пропуск реплея — это находка №85, повторённая пятый раз"
fi
bash $SETUP/attacks/replay-gate.sh "$C295_DIR" "$C296_DIR" "$C297_DIR" 2>&1 | tee /root/replay-2.9.8.txt
...
grep -q '88\.6/мин' /root/replay-2.9.8.txt \
    || die "реплей 4/4 не напечатал 88.6/мин — крит. 9 на collect-2.9.7 не воспроизвёл известный ответ (5.9.8e, №90)"
echo "реплей 4/4 пройден в $(date -u +%H:%M:%S) UTC"
...
date -u +"%Y-%m-%d %H:%M:%S" > /root/agent-start-2.9.8.txt
...
cd $SETUP/attacks
bash ./run-all-attacks.sh --dns-fd-reuse-controls 2>&1 | tee /root/dns-controls-2.9.8.txt
...
cd $SETUP/attacks
bash ./run-all-attacks.sh --counting-control 2>&1 | tee /root/counting-control-2.9.8.txt
...
cd $SETUP/attacks
bash ./run-all-attacks.sh --ringbuf-overflow 2>&1 | tee /root/ringbuf-overflow-2.9.8.txt
...
if [ "${SMOKE_ONLY:-0}" = "1" ]; then
    ...
    echo "=== ПРЕФИКС №2.9.8 ЗАВЕРШЁН $(date -u +%H:%M:%S) UTC ==="
    touch /root/PIPELINE-2.9.8-DONE
    exit 0
fi
...
export AGENT_START_FILE=/root/agent-start-2.9.8.txt
bash ./run-gate.sh 2>&1 | tee /root/gate-2.9.8.txt
GATE_RC=${PIPESTATUS[0]}
echo "гейт вернул $GATE_RC"
# Пункты 1-12 постановки №2.9.8 не подлежат толкованию — вытаскиваем их
...
if [ -f "$SETUP/run-2.9.8-report.sh" ]; then
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.8.txt \
        bash $SETUP/run-2.9.8-report.sh 2>&1 | tee /root/report-2.9.8.txt
else
    echo "ВНИМАНИЕ: run-2.9.8-report.sh отсутствует — считаем отчётом №2.9.5."
    echo "ВНИМАНИЕ: п.10 (idle-алерты на метках срезов) он посчитает, величины"
    echo "ВНИМАНИЕ: постановки №2.9.8 (пп.1-12) — нет; они только в гейте."
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.8.txt \
        bash $SETUP/run-2.9.5-report.sh 2>&1 | tee /root/report-2.9.8.txt
fi

echo "=== ПАЙПЛАЙН №2.9.8 ЗАВЕРШЁН $(date -u +%H:%M:%S) UTC, гейт=$GATE_RC ==="
touch /root/PIPELINE-2.9.8-DONE
