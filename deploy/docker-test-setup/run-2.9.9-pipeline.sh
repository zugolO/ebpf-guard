#!/usr/bin/env bash
# ЗАМЕР №2.9.9 — приёмка волны 5.9.9, вход в волну 6 (plan.md, "ЗАМЕР №2.9.9").
#
...
#   setsid nohup bash /opt/ebpf-guard/deploy/docker-test-setup/run-2.9.9-pipeline.sh >/dev/null 2>&1 &
# Готовность: файл /root/PIPELINE-2.9.9-DONE. Полный лог: /root/run-2.9.9-pipeline.log
#
...
exec > /root/run-2.9.9-pipeline.log 2>&1
set -x
SETUP=/opt/ebpf-guard/deploy/docker-test-setup
IDLE_OUT=$SETUP/idle-results/idle-2.9.9
P0_OUT=$SETUP/idle-results/p0-3-2.9.9
export PATH="$PATH:/usr/local/go/bin"

die() {
    echo "=== СТОП: $* ==="
    echo "=== ЗАМЕР №2.9.9 НЕ НАЧАТ. $(date -u +%H:%M:%S) UTC ==="
    touch /root/PIPELINE-2.9.9-DONE
    exit 1
}
...
echo "=== [1/14] ЖЁСТКИЙ СТОП №1: replay-gate.sh на ЧЕТЫРЁХ архивах (5.9.7c/5.9.8e/5.9.9a) ==="
...
# TIMESTAMP. Это пятый жёсткий стоп преflight'а по постановке №2.9.9; реплей,
# давший только один из двух исходов, останавливает цепочку здесь.
find_archive() {
    local name="$1" d
...
if [ -z "$C295_DIR" ] || [ -z "$C296_DIR" ] || [ -z "$C297_DIR" ] || [ -z "$C298_DIR" ]; then
    die "архивы collect-2.9.5/2.9.6/2.9.7/2.9.8 не найдены на стенде. Скопировать (например в /root/) либо задать REPLAY_C295_DIR/REPLAY_C296_DIR/REPLAY_C297_DIR/REPLAY_C298_DIR. Пропуск реплея — это находка №85, повторённая шестой раз"
fi
bash $SETUP/attacks/replay-gate.sh "$C295_DIR" "$C296_DIR" "$C297_DIR" "$C298_DIR" 2>&1 | tee /root/replay-2.9.9.txt
...
grep -q '88\.6/мин' /root/replay-2.9.9.txt \
    || die "реплей 4/5 не напечатал 88.6/мин — крит. 9 на collect-2.9.7 не воспроизвёл известный ответ (5.9.8e, №90)"
...
grep -q 'повторный вызов за тот же замер молчит' /root/replay-2.9.9.txt \
    || die "реплей 5/5: повторный вызов гейта за тот же замер не промолчал — самонаведение №99 не починено (5.9.9a)"
grep -q 'умеет падать, а не только молчать (5.9.9a)' /root/replay-2.9.9.txt \
    || die "реплей 5/5: с подменённым TIMESTAMP WARN «база брошена» не напечатан — механизм 5.9.6f ослеплён правкой 5.9.9a"
echo "реплей 5/5 пройден в $(date -u +%H:%M:%S) UTC"
...
date -u +"%Y-%m-%d %H:%M:%S" > /root/agent-start-2.9.9.txt
...
cd $SETUP/attacks
bash ./run-all-attacks.sh --counting-control 2>&1 | tee /root/counting-control-2.9.9.txt
...
cd $SETUP/attacks
bash ./run-all-attacks.sh --ringbuf-overflow 2>&1 | tee /root/ringbuf-overflow-2.9.9.txt
...
if [ "${SMOKE_ONLY:-0}" = "1" ]; then
    ...
    echo "=== ПРЕФИКС №2.9.9 ЗАВЕРШЁН $(date -u +%H:%M:%S) UTC ==="
    touch /root/PIPELINE-2.9.9-DONE
    exit 0
fi
...
export AGENT_START_FILE=/root/agent-start-2.9.9.txt
bash ./run-gate.sh 2>&1 | tee /root/gate-2.9.9.txt
GATE_RC=${PIPESTATUS[0]}
echo "гейт вернул $GATE_RC"
# Пункты 1-12 постановки №2.9.9 не подлежат толкованию — вытаскиваем их
...
# №2.9.6…№2.9.8 тоже). REPORT_LABEL=2.9.9 чинит это для report-2.9.9.txt;
# если когда-нибудь появится собственный run-2.9.9-report.sh, ему тоже
# следует читать REPORT_LABEL, а не зашивать номер.
if [ -f "$SETUP/run-2.9.9-report.sh" ]; then
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.9.txt REPORT_LABEL=2.9.9 \
        bash $SETUP/run-2.9.9-report.sh 2>&1 | tee /root/report-2.9.9.txt
else
    echo "ВНИМАНИЕ: run-2.9.9-report.sh отсутствует — считаем отчётом (сверх гейта) фолбэком run-2.9.5-report.sh."
    echo "ВНИМАНИЕ: п.10 (idle-алерты на метках срезов) он посчитает, величины"
    echo "ВНИМАНИЕ: постановки №2.9.9 (пп.1-12) — нет; они только в гейте."
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.9.txt REPORT_LABEL=2.9.9 \
        bash $SETUP/run-2.9.5-report.sh 2>&1 | tee /root/report-2.9.9.txt
fi

echo "=== ПАЙПЛАЙН №2.9.9 ЗАВЕРШЁН $(date -u +%H:%M:%S) UTC, гейт=$GATE_RC ==="
touch /root/PIPELINE-2.9.9-DONE
