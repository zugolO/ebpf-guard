#!/usr/bin/env bash
# ЗАМЕР №2.9.9.F — приёмка волны 5.9.9.F, вход в волну 6 (plan.md, "ЗАМЕР №2.9.9.F").
...
#   setsid nohup bash /opt/ebpf-guard/deploy/docker-test-setup/run-2.9.9.F-pipeline.sh >/dev/null 2>&1 &
# Готовность: файл /root/PIPELINE-2.9.9.F-DONE. Полный лог: /root/run-2.9.9.F-pipeline.log
#
...
exec > /root/run-2.9.9.F-pipeline.log 2>&1
set -x
SETUP=/opt/ebpf-guard/deploy/docker-test-setup
IDLE_OUT=$SETUP/idle-results/idle-2.9.9.F
P0_OUT=$SETUP/idle-results/p0-3-2.9.9.F
export PATH="$PATH:/usr/local/go/bin"

die() {
    echo "=== СТОП: $* ==="
    echo "=== ЗАМЕР №2.9.9.F НЕ НАЧАТ. $(date -u +%H:%M:%S) UTC ==="
...
# постановке №2.9.9.F; реплей, давший только один из двух исходов в любой из
...
if [ -z "$C295_DIR" ] || [ -z "$C296_DIR" ] || [ -z "$C297_DIR" ] || [ -z "$C298_DIR" ] || [ -z "$C299_DIR" ]; then
    die "архивы collect-2.9.5/2.9.6/2.9.7/2.9.8/2.9.9 не найдены на стенде. Скопировать (например в /root/) либо задать REPLAY_C295_DIR/REPLAY_C296_DIR/REPLAY_C297_DIR/REPLAY_C298_DIR/REPLAY_C299_DIR. Пропуск реплея — это находка №85, повторённая седьмой раз"
fi
bash $SETUP/attacks/replay-gate.sh "$C295_DIR" "$C296_DIR" "$C297_DIR" "$C298_DIR" "$C299_DIR" 2>&1 | tee /root/replay-2.9.9.F.txt
...
grep -q '88\.6/мин' /root/replay-2.9.9.F.txt \
    || die "реплей 4/6 не напечатал 88.6/мин — крит. 9 на collect-2.9.7 не воспроизвёл известный ответ (5.9.8e, №90)"
...
grep -q 'повторный вызов за тот же замер молчит' /root/replay-2.9.9.F.txt \
    || die "реплей 5/6: повторный вызов гейта за тот же замер не промолчал — самонаведение №99 не починено (5.9.9a)"
grep -q 'умеет падать, а не только молчать (5.9.9a)' /root/replay-2.9.9.F.txt \
    || die "реплей 5/6: с подменённым TIMESTAMP WARN «база брошена» не напечатан — механизм 5.9.6f ослеплён правкой 5.9.9a"
...
grep -q 'выросло за idle-час' /root/replay-2.9.9.F.txt \
    || die "реплей 6/6: вторая ветка спасения фонового правила («выросло за idle-час») не напечатана (5.9.9.Fe)"
grep -q 'напечатаны разными формулировками' /root/replay-2.9.9.F.txt \
    || die "реплей 6/6: вторая и третья ветки спасения не различены на одном прогоне — ветка idle_prewindow_list не проверена (5.9.9.Fe, №111)"
echo "реплей 6/6 пройден в $(date -u +%H:%M:%S) UTC"
...
date -u +"%Y-%m-%d %H:%M:%S" > /root/agent-start-2.9.9.F.txt
...
for dns_try in $(seq 1 "$DNS_CTL_ATTEMPTS"); do
    ...
    bash ./run-all-attacks.sh --dns-fd-reuse-controls 2>&1 | tee /root/dns-controls-2.9.9.F-$dns_try.txt
...
cd $SETUP/attacks
bash ./run-all-attacks.sh --counting-control 2>&1 | tee /root/counting-control-2.9.9.F.txt
...
cd $SETUP/attacks
bash ./run-all-attacks.sh --ringbuf-overflow 2>&1 | tee /root/ringbuf-overflow-2.9.9.F.txt
...
export AGENT_START_FILE=/root/agent-start-2.9.9.F.txt
bash ./run-gate.sh 2>&1 | tee /root/gate-2.9.9.F.txt
GATE_RC=${PIPESTATUS[0]}
echo "гейт вернул $GATE_RC"
# Пункты 1-14 постановки №2.9.9.F не подлежат толкованию — вытаскиваем их
...
# №2.9.6…№2.9.8 тоже). REPORT_LABEL=2.9.9.F чинит это для report-2.9.9.F.txt;
# если когда-нибудь появится собственный run-2.9.9.F-report.sh, ему тоже
# следует читать REPORT_LABEL, а не зашивать номер.
if [ -f "$SETUP/run-2.9.9.F-report.sh" ]; then
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.9.F.txt REPORT_LABEL=2.9.9.F \
        bash $SETUP/run-2.9.9.F-report.sh 2>&1 | tee /root/report-2.9.9.F.txt
else
    echo "ВНИМАНИЕ: run-2.9.9.F-report.sh отсутствует — считаем отчётом (сверх гейта) фолбэком run-2.9.5-report.sh."
    echo "ВНИМАНИЕ: п.10 (idle-алерты на метках срезов) он посчитает, величины"
    echo "ВНИМАНИЕ: постановки №2.9.9.F (пп.1-14) — нет; они только в гейте."
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.9.F.txt REPORT_LABEL=2.9.9.F \
        bash $SETUP/run-2.9.5-report.sh 2>&1 | tee /root/report-2.9.9.F.txt
fi

echo "=== ПАЙПЛАЙН №2.9.9.F ЗАВЕРШЁН $(date -u +%H:%M:%S) UTC, гейт=$GATE_RC ==="
touch /root/PIPELINE-2.9.9.F-DONE
