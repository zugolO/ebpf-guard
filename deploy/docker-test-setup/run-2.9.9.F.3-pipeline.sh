#!/usr/bin/env bash
# ЗАМЕР №2.9.9.F.3 — приёмка волны 5.9.9.F.3 (plan.md, "Волна 5.9.9.F.3"),
# первая ПРОДУКТОВАЯ волна после трёх измерительных подряд.
#
...
#   setsid nohup bash /opt/ebpf-guard/deploy/docker-test-setup/run-2.9.9.F.3-pipeline.sh >/dev/null 2>&1 &
# Предпрогон (~25 минут, без idle-часа и атак):
#   SMOKE_ONLY=1 setsid nohup bash .../run-2.9.9.F.3-pipeline.sh >/dev/null 2>&1 &
# Готовность: файл /root/PIPELINE-2.9.9.F.3-DONE. Полный лог: /root/run-2.9.9.F.3-pipeline.log
#
...
exec > /root/run-2.9.9.F.3-pipeline.log 2>&1
set -x
SETUP=/opt/ebpf-guard/deploy/docker-test-setup
IDLE_OUT=$SETUP/idle-results/idle-2.9.9.F.3
P0_OUT=$SETUP/idle-results/p0-3-2.9.9.F.3
export PATH="$PATH:/usr/local/go/bin"

die() {
    echo "=== СТОП: $* ==="
    echo "=== ЗАМЕР №2.9.9.F.3 НЕ НАЧАТ. $(date -u +%H:%M:%S) UTC ==="
...
export WAVE_CRITERIA_IDS="${WAVE_CRITERIA_IDS:-5.9.9.F.3a 5.9.9.F.3b 5.9.9.F.3c 5.9.9.F.3d 5.9.9.F.3e 5.9.9.F.3f 5.9.9.F.3g}"
...
export FP_CONTROL_TAG="${FP_CONTROL_TAG:-ebpfguard-f3-$(date -u +%Y%m%d_%H%M%S)}"
...
echo "--- преflight: 5.9.9.F.3c — признак песочницы systemd структурный, а не список comm (№133) ---"
...
for r in container_escape_mount container_escape_chroot container_escape_unshare_user \
         container_escape_cap_sys_admin privesc_unshare_user_ns cis_5_2_1_privileged_container; do
    body=$(for y in rules/*.yaml; do rule_predicate "$r" "$y"; done)
    [ -n "$body" ] || die "5.9.9.F.3c: правило $r не найдено в rules/*.yaml — шесть правил пункта обязаны остаться на месте, удаление приёмкой не является"
...
# либо признак есть у всех шести, либо переменная объявила дробление явно.
if [ "$f3c_hit" -lt 6 ] && [ "${WAVE_F3C_SPLIT:-0}" != "1" ]; then
    die "5.9.9.F.3c: структурный признак песочницы systemd есть только у $f3c_hit правил из 6, и дробление пункта НЕ объявлено (WAVE_F3C_SPLIT=1). Постановка требует объявлять дробление до правки, а не после FAIL — иначе замер измерит половину волны и назовёт это приёмкой"
fi
[ "$f3c_hit" -ge 1 ] \
    || die "5.9.9.F.3c: ни у одного из шести правил нет структурного признака песочницы — правка №133 не в дереве вовсе, и idle-час снова даст три verdict=\"attack\" на старте любого sandboxed-юнита"
...
echo "  ок: 5.9.9.F.3c в дереве ($f3c_hit/6 правил со структурным признаком)"
...
awk '/^[[:space:]]*if \[ "\$in_induced" -eq 1 \]; then/{print NR": induced"} /^[[:space:]]*elif \[ -n "\$phase_parts" \]; then/{print NR": phase_parts"} /^[[:space:]]*elif \[ -z "\$IDLE_METRICS_START" \]/{print NR": fallback"}' \
    $SETUP/attacks/run-gate.sh | sed 's/^/  порядок веток: /'
...
bash $SETUP/attacks/replay-gate.sh "$C295_DIR" "$C296_DIR" "$C297_DIR" "$C298_DIR" "$C299_DIR" "$C299F_DIR" "$C299F1_DIR" "$C299F2_DIR" 2>&1 | tee /root/replay-2.9.9.F.3.txt
...
grep -q 'случай 1 (алерт старше регистрации корня), смещение -26мс напечатано' /root/replay-2.9.9.F.3.txt \
...
grep -q 'критерий 16 умеет падать, а не только пропускать' /root/replay-2.9.9.F.3.txt \
...
grep -q 'ноль 5.9.9.F.1c объясняется печатью, а не допущением' /root/replay-2.9.9.F.3.txt \
...
grep -q 'ветка реестра различает объяснённый ноль и регресс детекта' /root/replay-2.9.9.F.3.txt \
...
grep -q 'без AGENT_START_FILE крит. 17 = SKIP «окно журнала не задано», вердикта нет' /root/replay-2.9.9.F.3.txt \
...
grep -q 'оба исхода 5.9.9.Fc воспроизведены на одном архиве' /root/replay-2.9.9.F.3.txt \
...
[ "$(grep -c 'состав idle-часа целиком покрыт idle-actors.txt, новых акторов 0' /root/replay-2.9.9.F.3.txt)" -ge 2 ] \
...
grep -q 'вне idle-actors.txt — крит. FAIL и назван поимённо' /root/replay-2.9.9.F.3.txt \
...
grep -q 'без journalctl крит. 5.9.4h деградирует к старому «0» текстом, не падает' /root/replay-2.9.9.F.3.txt \
...
grep -q 'называет 11 правил категорией (а), «немых правил: 0» не печатается' /root/replay-2.9.9.F.3.txt \
...
grep -q 'считает замкнутым тождеством с симметричным ±1500 и даёт PASS на исправном прогоне' /root/replay-2.9.9.F.3.txt \
...
grep -q 'замкнутое тождество умеет падать' /root/replay-2.9.9.F.3.txt \
...
grep -q 'фаза idle не поглощается меткой преflight' /root/replay-2.9.9.F.3.txt \
...
grep -q 'четвёртая фаза сохранена' /root/replay-2.9.9.F.3.txt \
...
grep -q 'SKIP=0 на архиве collect-2.9.9.F.2 без правки критериев' /root/replay-2.9.9.F.3.txt \
    || die "реплей 14/14: гейт на архиве 2.9.9.F.2 не дал SKIP=0 — три дефекта бухгалтерии (№135/№136/№137) не сняты, и обязательная величина приёмки №1 недостижима на замере ровно по той же причине, по какой она не достигнута на №2.9.9.F.2"
grep -q 'счётчик неисполнившихся веток умеет расти' /root/replay-2.9.9.F.3.txt \
...
date -u +"%Y-%m-%d %H:%M:%S" > /root/agent-start-2.9.9.F.3.txt
...
for dns_try in $(seq 1 "$DNS_CTL_ATTEMPTS"); do
    ...
    bash ./run-all-attacks.sh --dns-fd-reuse-controls 2>&1 | tee /root/dns-controls-2.9.9.F.3-$dns_try.txt
...
cd $SETUP/attacks
bash ./run-all-attacks.sh --counting-control 2>&1 | tee /root/counting-control-2.9.9.F.3.txt
...
cd $SETUP/attacks
bash ./run-all-attacks.sh --ringbuf-overflow 2>&1 | tee /root/ringbuf-overflow-2.9.9.F.3.txt
...
EBPF_GUARD_API="$CPU_API" EBPF_GUARD_TOKEN="$CPU_TOKEN" \
    bash ./run-all-attacks.sh --cpu-pressure-control 2>&1 | tee /root/cpu-pressure-2.9.9.F.3.txt
...
for r in rootkit_hidden_dir_dev privesc_suid_suspicious_path sigma_binary_in_tmp_executed \
    ...
    echo "$fp_rules" | jq -e --arg r "$r" '[.. | objects | select(.id? == $r)] | length > 0' >/dev/null 2>&1 \
        || die "5.9.9.F.3: правило $r не найдено в /api/v1/rules — оно не загрузилось, и его ноль на приёмке означал бы битый YAML, а не сужение (проверить синтаксис правки)"
done
echo "  все девять правых правил загружены агентом (не битый YAML)"
...
# Негатив: старт настоящего sandboxed-юнита. man-db.service выбран потому,
# что именно он дал пакет 11:57:39 (PrivateTmp + PrivateDevices + три
# дочерних процесса, 56 алертов за 250 мс). Позитив: unshare -Urm из
# процесса, чей ppid НЕ 1, — то есть настоящий побег, которого сужение
# ослепить не имеет права.
echo "--- 5.9.9.F.3c: пара контролей песочницы systemd (№133) ---"
...
c_inc_before=$(fp_count incident_confirmed_attack)
...
if systemctl list-unit-files "$fp_unit" >/dev/null 2>&1 && [ -n "$(systemctl list-unit-files --no-legend "$fp_unit" 2>/dev/null)" ]; then
    ...
    c_inc_after=$(fp_count incident_confirmed_attack)
    echo "  негативный: systemctl start $fp_unit -> Δ(шесть правил побега)=$((c_sum_after - c_sum_before)) Δincident_confirmed_attack=$((c_inc_after - c_inc_before))"
    ...
    [ "$((c_inc_after - c_inc_before))" -eq 0 ] \
        || die "5.9.9.F.3c ПРОВАЛЕН: старт $fp_unit всё ещё промотирован до incident_confirmed_attack (Δ$((c_inc_after - c_inc_before))) — шесть правил замолчали, но корреляция набирает score из оставшихся (sensitive_file_read, drift_*, rootkit_hidden_dir_dev). Пункт 3c закрыт наполовину"
else
...
if [ "${SMOKE_ONLY:-0}" = "1" ]; then
    ...
    EBPF_GUARD_API="$CRED_API" EBPF_GUARD_TOKEN="$CRED_TOKEN" \
        bash ./run-all-attacks.sh --cred-proc-maps-control 2>&1 | tee /root/cred-proc-maps-2.9.9.F.3.txt
...
if [ "${SMOKE_ONLY:-0}" = "1" ]; then
    ...
    echo "=== ПРЕДПРОГОН №2.9.9.F.3 ЗАВЕРШЁН $(date -u +%H:%M:%S) UTC ==="
    touch /root/PIPELINE-2.9.9.F.3-DONE
    exit 0
fi
...
export AGENT_START_FILE=/root/agent-start-2.9.9.F.3.txt
bash ./run-gate.sh 2>&1 | tee /root/gate-2.9.9.F.3.txt
GATE_RC=${PIPESTATUS[0]}
echo "гейт вернул $GATE_RC"
# Пункты 1-14 постановки №2.9.9.F.3 не подлежат толкованию — вытаскиваем их
...
acc_line=$(grep -E '^RUN-GATE: PASS=' /root/gate-2.9.9.F.3.txt | tail -1)
acc_skip=$(echo "$acc_line" | sed -E 's/.*SKIP=([0-9]+).*/\1/')
acc_verdict=$(grep -cE '^\[FAIL\]|\[0;31m\[FAIL\]' /root/gate-2.9.9.F.3.txt || true)
if [ "${acc_skip:-99}" = "0" ] && [ "$GATE_RC" -eq 0 ]; then
    ...
    grep -E '\[SKIP\]|\[FAIL\]' /root/gate-2.9.9.F.3.txt | sed 's/^/       /'
    grep -E 'чья ветка не исполнилась' /root/gate-2.9.9.F.3.txt | sed 's/^/       /'
fi
...
grep -E 'verdict="attack" за idle-час|алертов за idle-час|состав промоушенов по comm' /root/gate-2.9.9.F.3.txt | sed 's/^/       /'
acc_idle_alerts=$(grep -oE 'алертов за idle-час: [0-9]+' /root/gate-2.9.9.F.3.txt | tail -1 | grep -oE '[0-9]+$')
acc_idle_att=$(grep -oE 'verdict="attack" за idle-час: [0-9]+ -> [0-9]+, дельта = [0-9-]+' /root/gate-2.9.9.F.3.txt | tail -1 | grep -oE '[0-9-]+$')
...
grep -E 'новые акторы idle-часа вне idle-actors|новых акторов 0|состав ВСЕХ новых алертов idle-часа' /root/gate-2.9.9.F.3.txt | sed 's/^/       /'
if grep -q 'новый(е) актор(ы) idle-часа вне idle-actors.txt' /root/gate-2.9.9.F.3.txt; then
...
grep -E 'добавлено по фазам|наведено шагом преflight' /root/gate-2.9.9.F.3.txt | head -14 | sed 's/^/       /'
acc_induced=$(grep -oE 'наведено преflight-шагом=[0-9]+' /root/gate-2.9.9.F.3.txt | tail -1 | grep -oE '[0-9]+$')
...
echo "=== конец приёмки волны 5.9.9.F.3 (вердикт гейта: $GATE_RC) ==="
echo ""
...
# №2.9.6…№2.9.8 тоже). REPORT_LABEL=2.9.9.F.3 чинит это для report-2.9.9.F.3.txt;
# если когда-нибудь появится собственный run-2.9.9.F.3-report.sh, ему тоже
# следует читать REPORT_LABEL, а не зашивать номер.
if [ -f "$SETUP/run-2.9.9.F.3-report.sh" ]; then
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.9.F.3.txt REPORT_LABEL=2.9.9.F.3 \
        bash $SETUP/run-2.9.9.F.3-report.sh 2>&1 | tee /root/report-2.9.9.F.3.txt
else
    echo "ВНИМАНИЕ: run-2.9.9.F.3-report.sh отсутствует — считаем отчётом (сверх гейта) фолбэком run-2.9.5-report.sh."
    echo "ВНИМАНИЕ: п.10 (idle-алерты на метках срезов) он посчитает, величины"
    echo "ВНИМАНИЕ: постановки №2.9.9.F.3 (пп.1-14) — нет; они только в гейте."
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.9.F.3.txt REPORT_LABEL=2.9.9.F.3 \
