#!/usr/bin/env bash
# ЗАМЕР №2.9.9.F.4 — приёмка волны 5.9.9.F.4 (plan.md, "Волна 5.9.9.F.4"),
# переехал из run-2.9.9.F.3-pipeline.sh (замер №2.9.9.F.3 ОТКЛОНЁН, пайплайн
# оборвался на шаге [9.6/14] до открытия baseline — см. таблицу волны в
# plan.md). Файл — дословная копия отклонённого предшественника с двумя
# правками, исполненными первыми по порядку волны (5.9.9.F.4b, затем
# 5.9.9.F.4a — сначала измеритель, потом продукт, тем же приёмом, каким
# сама волна упорядочена в постановке):
#
...
#     затирал их архив).
#
# ЗАМЕР №2.9.9.F.4 — приёмка волн 5.9.9.F.4 И 5.9.9.F.3 сразу (plan.md,
# "Волна 5.9.9.F.4"; пункт 4i переносит все семь пунктов 5.9.9.F.3 внутрь
# этой приёмки — замер №2.9.9.F.3 оборвался на шаге [9.6/14] и отклонён).
#
...
#   setsid nohup bash /opt/ebpf-guard/deploy/docker-test-setup/run-2.9.9.F.4-pipeline.sh >/dev/null 2>&1 &
# Предпрогон (~25 минут, без idle-часа и атак):
#   SMOKE_ONLY=1 setsid nohup bash .../run-2.9.9.F.4-pipeline.sh >/dev/null 2>&1 &
# Готовность: файл /root/PIPELINE-2.9.9.F.4-DONE. Полный лог: /root/run-2.9.9.F.4-pipeline.log
#
...
exec > /root/run-2.9.9.F.4-pipeline.log 2>&1
set -x
SETUP=/opt/ebpf-guard/deploy/docker-test-setup
IDLE_OUT=$SETUP/idle-results/idle-2.9.9.F.4
P0_OUT=$SETUP/idle-results/p0-3-2.9.9.F.4
export PATH="$PATH:/usr/local/go/bin"

die() {
    echo "=== СТОП: $* ==="
    echo "=== ЗАМЕР №2.9.9.F.4 НЕ НАЧАТ. $(date -u +%H:%M:%S) UTC ==="
    ...
    touch /root/PIPELINE-2.9.9.F.4-DONE
    exit 1
}
...
release_pipeline_observer_root() {
    ...
    echo "5.9.9.F.4c: root_pid обнулён — граница окна замера ($(date -u +%H:%M:%S) UTC), baseline и окно атак видят стенд без исключения дерева пайплайна"
}
register_pipeline_observer_root
...
export WAVE_CRITERIA_IDS="${WAVE_CRITERIA_IDS:-5.9.9.F.3a 5.9.9.F.3b 5.9.9.F.3c 5.9.9.F.3d 5.9.9.F.3e 5.9.9.F.3f 5.9.9.F.3g 5.9.9.F.4a 5.9.9.F.4b 5.9.9.F.4c 5.9.9.F.4d 5.9.9.F.4e 5.9.9.F.4f 5.9.9.F.4g 5.9.9.F.4h 5.9.9.F.4i}"
...
bash $SETUP/attacks/replay-gate.sh "$C295_DIR" "$C296_DIR" "$C297_DIR" "$C298_DIR" "$C299_DIR" "$C299F_DIR" "$C299F1_DIR" "$C299F2_DIR" 2>&1 | tee /root/replay-2.9.9.F.4.txt
...
grep -q 'случай 1 (алерт старше регистрации корня), смещение -26мс напечатано' /root/replay-2.9.9.F.4.txt \
...
grep -q 'критерий 16 умеет падать, а не только пропускать' /root/replay-2.9.9.F.4.txt \
...
grep -q 'ноль 5.9.9.F.1c объясняется печатью, а не допущением' /root/replay-2.9.9.F.4.txt \
...
grep -q 'ветка реестра различает объяснённый ноль и регресс детекта' /root/replay-2.9.9.F.4.txt \
...
grep -q 'без AGENT_START_FILE крит. 17 = SKIP «окно журнала не задано», вердикта нет' /root/replay-2.9.9.F.4.txt \
...
grep -q 'оба исхода 5.9.9.Fc воспроизведены на одном архиве' /root/replay-2.9.9.F.4.txt \
...
[ "$(grep -c 'состав idle-часа целиком покрыт idle-actors.txt, новых акторов 0' /root/replay-2.9.9.F.4.txt)" -ge 2 ] \
...
grep -q 'вне idle-actors.txt — крит. FAIL и назван поимённо' /root/replay-2.9.9.F.4.txt \
...
grep -q 'без journalctl крит. 5.9.4h деградирует к старому «0» текстом, не падает' /root/replay-2.9.9.F.4.txt \
...
grep -q 'называет 11 правил категорией (а), «немых правил: 0» не печатается' /root/replay-2.9.9.F.4.txt \
...
grep -q 'считает замкнутым тождеством с симметричным ±1500 и даёт PASS на исправном прогоне' /root/replay-2.9.9.F.4.txt \
...
grep -q 'замкнутое тождество умеет падать' /root/replay-2.9.9.F.4.txt \
...
grep -q 'фаза idle не поглощается меткой преflight' /root/replay-2.9.9.F.4.txt \
...
grep -q 'четвёртая фаза сохранена' /root/replay-2.9.9.F.4.txt \
...
grep -q 'три неисполнившиеся ветки архива (5.9.9.Fc/5.9.9.F.1d/5.9.9.F.2b) исполняются без правки самих критериев' /root/replay-2.9.9.F.4.txt \
...
grep -q 'счётчик неисполнившихся веток умеет расти' /root/replay-2.9.9.F.4.txt \
...
register_pipeline_observer_root
date -u +"%Y-%m-%d %H:%M:%S" > /root/agent-start-2.9.9.F.4.txt
...
for dns_try in $(seq 1 "$DNS_CTL_ATTEMPTS"); do
    ...
    bash ./run-all-attacks.sh --dns-fd-reuse-controls 2>&1 | tee /root/dns-controls-2.9.9.F.4-$dns_try.txt
...
cd $SETUP/attacks
bash ./run-all-attacks.sh --counting-control 2>&1 | tee /root/counting-control-2.9.9.F.4.txt
...
cd $SETUP/attacks
bash ./run-all-attacks.sh --ringbuf-overflow 2>&1 | tee /root/ringbuf-overflow-2.9.9.F.4.txt
...
EBPF_GUARD_API="$CPU_API" EBPF_GUARD_TOKEN="$CPU_TOKEN" \
    bash ./run-all-attacks.sh --cpu-pressure-control 2>&1 | tee /root/cpu-pressure-2.9.9.F.4.txt
...
if [ "${SMOKE_ONLY:-0}" = "1" ]; then
    ...
    EBPF_GUARD_API="$CRED_API" EBPF_GUARD_TOKEN="$CRED_TOKEN" \
        bash ./run-all-attacks.sh --cred-proc-maps-control 2>&1 | tee /root/cred-proc-maps-2.9.9.F.4.txt
...
if [ "${SMOKE_ONLY:-0}" = "1" ]; then
    ...
    echo "=== ПРЕДПРОГОН №2.9.9.F.4 ЗАВЕРШЁН $(date -u +%H:%M:%S) UTC ==="
...
export AGENT_START_FILE=/root/agent-start-2.9.9.F.4.txt
bash ./run-gate.sh 2>&1 | tee /root/gate-2.9.9.F.4.txt
GATE_RC=${PIPESTATUS[0]}
echo "гейт вернул $GATE_RC"
# Пункты 1-14 постановки №2.9.9.F.4 не подлежат толкованию — вытаскиваем их
...
acc_line=$(grep -E '^RUN-GATE: PASS=' /root/gate-2.9.9.F.4.txt | tail -1)
acc_skip=$(echo "$acc_line" | sed -E 's/.*SKIP=([0-9]+).*/\1/')
acc_verdict=$(grep -cE '^\[FAIL\]|\[0;31m\[FAIL\]' /root/gate-2.9.9.F.4.txt || true)
if [ "${acc_skip:-99}" = "0" ] && [ "$GATE_RC" -eq 0 ]; then
    ...
    grep -E '\[SKIP\]|\[FAIL\]' /root/gate-2.9.9.F.4.txt | sed 's/^/       /'
    grep -E 'чья ветка не исполнилась' /root/gate-2.9.9.F.4.txt | sed 's/^/       /'
fi
...
grep -E 'verdict="attack" за idle-час|алертов за idle-час|состав промоушенов по comm' /root/gate-2.9.9.F.4.txt | sed 's/^/       /'
acc_idle_alerts=$(grep -oE 'алертов за idle-час: [0-9]+' /root/gate-2.9.9.F.4.txt | tail -1 | grep -oE '[0-9]+$')
acc_idle_att=$(grep -oE 'verdict="attack" за idle-час: [0-9]+ -> [0-9]+, дельта = [0-9-]+' /root/gate-2.9.9.F.4.txt | tail -1 | grep -oE '[0-9-]+$')
...
grep -E 'новые акторы idle-часа вне idle-actors|новых акторов 0|состав ВСЕХ новых алертов idle-часа' /root/gate-2.9.9.F.4.txt | sed 's/^/       /'
if grep -q 'новый(е) актор(ы) idle-часа вне idle-actors.txt' /root/gate-2.9.9.F.4.txt; then
...
# (6) Фаза idle больше не поглощается меткой преflight'а (5.9.9.F.3d/№134).
acc "6." "фазы прироста состава детекта (5.9.9.F.3d/№134):"
grep -E 'добавлено по фазам|наведено шагом преflight' /root/gate-2.9.9.F.4.txt | head -14 | sed 's/^/       /'
acc_induced=$(grep -oE 'наведено преflight-шагом=[0-9]+' /root/gate-2.9.9.F.4.txt | tail -1 | grep -oE '[0-9]+$')
if [ "${acc_induced:-99}" -le 4 ] 2>/dev/null; then
    acc "  " "ДОСТИГНУТО: наведено преflight-шагом=${acc_induced} (<=4) — фаза idle больше не поглощается; на №2.9.9.F.2 было 11, из них 7 на деле idle"
else
    acc "  " "НЕ ДОСТИГНУТО: наведено преflight-шагом=${acc_induced:-?} — ветка in_induced по-прежнему поглощает idle (находка №134). Проверить, что порядок веток в run-gate.sh изменён, а не только описан"
fi
...
# (8) 5.9.9.F.4c/№147: доля алертов АПТАЙМА (acc_alerts — тот же снимок
# /api/v1/alerts, что и у величин 2/5, с рестарта [5/14] до этого момента,
# то есть idle-час + окно атак), приходящаяся на акторы измерителя и сборки.
# Список — поимённо из постановки волны (не восстанавливается по коду):
# curl, jq, grep, go, ld, clang, llvm-strip, bpf2go, *.test, cmdlinescan.
# `*.test` матчится суффиксом (go test компилирует бинарь вида
# pkgname.test), остальные — точным именем comm.
acc_harness_comms='["curl","jq","grep","go","ld","clang","llvm-strip","bpf2go","cmdlinescan"]'
acc_alerts_total=$(echo "$acc_alerts" | jq 'length' 2>/dev/null)
acc_harness_alerts=$(echo "$acc_alerts" | jq --argjson hc "$acc_harness_comms" \
    '[.[] | select((.comm // "") as $c | ($hc | index($c)) or ($c | test("\\.test$")))] | length' 2>/dev/null)
acc "8." "доля алертов аптайма от акторов измерителя и сборки (5.9.9.F.4c/№147): $acc_harness_alerts из $acc_alerts_total"
if [ "${acc_alerts_total:-0}" -gt 0 ] 2>/dev/null; then
    acc_harness_pct=$(awk -v h="${acc_harness_alerts:-0}" -v t="$acc_alerts_total" 'BEGIN{printf "%.1f", 100*h/t}')
    acc "  " "доля = ${acc_harness_pct}% (было 55% на отклонённом №2.9.9.F.3; цель < 5%)"
    if awk -v p="$acc_harness_pct" 'BEGIN{exit !(p < 5)}'; then
        acc "  " "ДОСТИГНУТО: доля < 5%"
    else
        acc "  " "НЕ ДОСТИГНУТО: доля >= 5% — состав по comm:"
        echo "$acc_alerts" | jq --argjson hc "$acc_harness_comms" -r \
            '[.[] | select((.comm // "") as $c | ($hc | index($c)) or ($c | test("\\.test$")))] | group_by(.comm) | .[] | "       \(length)\t\(.[0].comm)\t\(.[0].rule_id)"' 2>/dev/null | sort -rn | head -20
    fi
else
    acc "  " "SKIP: acc_alerts пуст или /api/v1/alerts недоступен — доля не посчитана"
fi
...
# [6/14]) — постановка требует напечатать остаток отдельной величиной
# наблюдения, а не списывать его молча на 4c.
acc_pid1_actors='["systemd","systemd-logind","dbus-daemon","polkitd","journald","systemd-journal","systemd-udevd"]'
acc_pid1_alerts=$(echo "$acc_alerts" | jq --argjson pc "$acc_pid1_actors" '[.[] | select((.comm // "") as $c | $pc | index($c))] | length' 2>/dev/null)
acc "  " "наблюдение без порога: алертов от акторов, наведённых pid 1 (systemctl/journalctl самого пайплайна, дерево-исключение их не ловит) = ${acc_pid1_alerts:-0}"
acc_excl_obs=$(grep -oE 'excluded\{observer_tree\}[^:]*: [0-9]+' /root/gate-2.9.9.F.4.txt | tail -1 | grep -oE '[0-9]+$')
...
acc "10." "exfil_archive_to_network_pipe за аптайм (5.9.9.F.4e/№145): $acc_exfil (было 96 — первое место по объёму), маркеров позитивного контроля: $acc_exfil_pos"
...
if [ "${acc_exfil_pos:-0}" = "0" ]; then
    ...
    acc "  " "НЕ ДОСТИГНУТО: контроль исполнился, а правило дало 0 — родительская форма tar->curl не поднимает правило вовсе (ослепление либо comm архиватора не доходит до proc.parent_comm)"
else
    acc "  " "ДОСТИГНУТО (в границе пункта): ненулевое на родительской форме; строки с parent вне списка архиваторов означали бы, что сужение не применилось"
fi
...
acc_stale=$(grep -oE 'stale_transitions за прогон: [0-9]+' /root/gate-2.9.9.F.4.txt | tail -1 | grep -oE '[0-9]+$')
...
# №2.9.6…№2.9.8 тоже). REPORT_LABEL=2.9.9.F.4 чинит это для report-2.9.9.F.4.txt;
# если когда-нибудь появится собственный run-2.9.9.F.4-report.sh, ему тоже
# следует читать REPORT_LABEL, а не зашивать номер.
if [ -f "$SETUP/run-2.9.9.F.4-report.sh" ]; then
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.9.F.4.txt REPORT_LABEL=2.9.9.F.4 \
        bash $SETUP/run-2.9.9.F.4-report.sh 2>&1 | tee /root/report-2.9.9.F.4.txt
else
    echo "ВНИМАНИЕ: run-2.9.9.F.4-report.sh отсутствует — считаем отчётом (сверх гейта) фолбэком run-2.9.5-report.sh."
    echo "ВНИМАНИЕ: п.10 (idle-алерты на метках срезов) он посчитает, величины"
    echo "ВНИМАНИЕ: постановки №2.9.9.F.4 (пп.1-14) — нет; они только в гейте."
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.9.F.4.txt REPORT_LABEL=2.9.9.F.4 \
        bash $SETUP/run-2.9.5-report.sh 2>&1 | tee /root/report-2.9.9.F.4.txt
fi
...
drift_out=/root/drift-composition-2.9.9.F.4.txt
{
    echo "=== состав drift_new_exec_critical за аптайм, замер №2.9.9.F.4, $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
...
echo "  наблюдение сложено в $drift_out (собирается вместе с /root/report-2.9.9.F.4.txt)"

echo "=== ПАЙПЛАЙН №2.9.9.F.4 ЗАВЕРШЁН $(date -u +%H:%M:%S) UTC, гейт=$GATE_RC ==="
touch /root/PIPELINE-2.9.9.F.4-DONE
