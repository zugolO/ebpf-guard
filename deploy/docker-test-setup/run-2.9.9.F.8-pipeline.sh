...
echo "=== [1/14] ЖЁСТКИЙ СТОП №1: replay-gate.sh на ДЕСЯТИ архивах, 16 реплеев (5.9.7c…5.9.9.F.7) ==="
...
echo "  архив 2.9.4 (реплеи 15/16, 16/16): ${C294_DIR:-НЕ НАЙДЕН}"
echo "  архив 2.9.9.F.6 (реплеи 15/16, 16/16): ${C299F6_DIR:-НЕ НАЙДЕН}"
if [ -z "$C295_DIR" ] || [ -z "$C296_DIR" ] || [ -z "$C297_DIR" ] || [ -z "$C298_DIR" ] || [ -z "$C299_DIR" ] || [ -z "$C299F_DIR" ] || [ -z "$C299F1_DIR" ] || [ -z "$C299F2_DIR" ] || [ -z "$C294_DIR" ] || [ -z "$C299F6_DIR" ]; then
    die "архивы collect-2.9.5…collect-2.9.9.F.2, collect-2.9.4, collect-2.9.9.F.6 найдены не все на стенде. Скопировать (например в /root/) либо задать REPLAY_C295_DIR…REPLAY_C299F2_DIR/REPLAY_C294_DIR/REPLAY_C299F6_DIR. Пропуск реплея — это находка №85, повторённая десятый раз"
fi
# Реплеи 15/16 и 16/16 читают idle-час и attacks/ обоих новых архивов. Их
...
[ -s "$C299F6_DIR/idle/alerts-end.json" ] \
    || die "реплей 15/16: $C299F6_DIR/idle/alerts-end.json отсутствует — сторона PASS крит. №68 (5.9.9.F.7b) непроверяема"
[ -s "$C294_DIR/idle/idle-2.9.4/alerts-end.json" ] \
    || die "реплей 15/16: $C294_DIR/idle/idle-2.9.4/alerts-end.json отсутствует — сторона FAIL крит. №68 (живой рецидив механизма) непроверяема"
...
bash $SETUP/attacks/replay-gate.sh "$C295_DIR" "$C296_DIR" "$C297_DIR" "$C298_DIR" "$C299_DIR" "$C299F_DIR" "$C299F1_DIR" "$C299F2_DIR" "$C294_DIR" "$C299F6_DIR" 2>&1 | tee /root/replay-2.9.9.F.8.txt
