#!/usr/bin/env bash
# ЗАМЕР №2.9.9.F.8 — приёмка волны 5.9.9.F.8 (plan.md, "Волна 5.9.9.F.8"),
# переехал из run-2.9.9.F.7-pipeline.sh. Волна 8 — ПОСЛЕДНЯЯ перед волной 6:
# два пункта, оба в измерителе, оба исполнены офлайн ДО замера; здесь они
# получают живые входы. Продуктовых правок в волне НЕТ намеренно.
#
...
#   setsid nohup bash /opt/ebpf-guard/deploy/docker-test-setup/run-2.9.9.F.8-pipeline.sh >/dev/null 2>&1 &
# Предпрогон (~25 минут, без idle-часа и атак):
#   SMOKE_ONLY=1 setsid nohup bash .../run-2.9.9.F.8-pipeline.sh >/dev/null 2>&1 &
# Готовность: файл /root/PIPELINE-2.9.9.F.8-DONE. Полный лог: /root/run-2.9.9.F.8-pipeline.log
#
...
exec > /root/run-2.9.9.F.8-pipeline.log 2>&1
set -x
SETUP=/opt/ebpf-guard/deploy/docker-test-setup
IDLE_OUT=$SETUP/idle-results/idle-2.9.9.F.8
P0_OUT=$SETUP/idle-results/p0-3-2.9.9.F.8
export PATH="$PATH:/usr/local/go/bin"

die() {
    echo "=== СТОП: $* ==="
    echo "=== ЗАМЕР №2.9.9.F.8 НЕ НАЧАТ. $(date -u +%H:%M:%S) UTC ==="
    ...
    touch /root/PIPELINE-2.9.9.F.8-DONE
    exit 1
}
...
bash $SETUP/attacks/replay-gate.sh "$C295_DIR" "$C296_DIR" "$C297_DIR" "$C298_DIR" "$C299_DIR" "$C299F_DIR" "$C299F1_DIR" "$C299F2_DIR" "$C294_DIR" "$C299F6_DIR" "$C299F7_DIR" 2>&1 | tee /root/replay-2.9.9.F.8.txt
...
grep -q 'случай 1 (алерт старше регистрации корня), смещение -26мс напечатано' /root/replay-2.9.9.F.8.txt \
...
grep -q 'критерий 16 умеет падать, а не только пропускать' /root/replay-2.9.9.F.8.txt \
...
grep -q 'ноль 5.9.9.F.1c объясняется печатью, а не допущением' /root/replay-2.9.9.F.8.txt \
...
grep -q 'ветка реестра различает объяснённый ноль и регресс детекта' /root/replay-2.9.9.F.8.txt \
...
grep -q 'без AGENT_START_FILE крит. 17 = SKIP «окно журнала не задано», вердикта нет' /root/replay-2.9.9.F.8.txt \
...
grep -q 'оба исхода 5.9.9.Fc воспроизведены на одном архиве' /root/replay-2.9.9.F.8.txt \
...
[ "$(grep -c 'состав idle-часа целиком покрыт idle-actors.txt, новых акторов 0' /root/replay-2.9.9.F.8.txt)" -ge 2 ] \
...
grep -q 'вне idle-actors.txt — крит. FAIL и назван поимённо' /root/replay-2.9.9.F.8.txt \
...
grep -q 'без journalctl крит. 5.9.4h деградирует к старому «0» текстом, не падает' /root/replay-2.9.9.F.8.txt \
...
grep -q 'называет 11 правил категорией (а), «немых правил: 0» не печатается' /root/replay-2.9.9.F.8.txt \
...
grep -q 'считает замкнутым тождеством с симметричным ±1500 и даёт PASS на исправном прогоне' /root/replay-2.9.9.F.8.txt \
...
grep -q 'замкнутое тождество умеет падать' /root/replay-2.9.9.F.8.txt \
...
grep -q 'фаза idle не поглощается меткой преflight' /root/replay-2.9.9.F.8.txt \
...
grep -q 'четвёртая фаза сохранена' /root/replay-2.9.9.F.8.txt \
...
grep -q 'три неисполнившиеся ветки архива (5.9.9.Fc/5.9.9.F.1d/5.9.9.F.2b) исполняются без правки самих критериев' /root/replay-2.9.9.F.8.txt \
...
grep -q 'счётчик неисполнившихся веток умеет расти' /root/replay-2.9.9.F.8.txt \
...
grep -qF 'collect-2.9.9.F.6: крит. №68 = PASS' /root/replay-2.9.9.F.8.txt \
...
grep -qF 'collect-2.9.4: крит. №68 = FAIL' /root/replay-2.9.9.F.8.txt \
...
grep -qF 'оба архива: крит. 9 печатает и долю по инцидентам, и долю по алертам' /root/replay-2.9.9.F.8.txt \
...
grep -q 'idle-час с fwupd целиком покрыт idle-actors.txt, новых акторов 0' /root/replay-2.9.9.F.8.txt \
...
grep -q 'без четырёх строк реестра крит. FAIL и fwupd/fwupdmgr названы поимённо' /root/replay-2.9.9.F.8.txt \
...
grep -q 'ветка классификации исполняется и даёт PASS' /root/replay-2.9.9.F.8.txt \
...
grep -q 'на collect-2.9.9.F.7 (флапа нет) — SKIP' /root/replay-2.9.9.F.8.txt \
...
register_pipeline_observer_root
date -u +"%Y-%m-%d %H:%M:%S" > /root/agent-start-2.9.9.F.8.txt
...
for dns_try in $(seq 1 "$DNS_CTL_ATTEMPTS"); do
    ...
    release_pipeline_observer_root
    bash ./run-all-attacks.sh --dns-fd-reuse-controls 2>&1 | tee /root/dns-controls-2.9.9.F.8-$dns_try.txt
    register_pipeline_observer_root
...
cd $SETUP/attacks
release_pipeline_observer_root
bash ./run-all-attacks.sh --counting-control 2>&1 | tee /root/counting-control-2.9.9.F.8.txt
register_pipeline_observer_root
...
cd $SETUP/attacks
release_pipeline_observer_root
bash ./run-all-attacks.sh --ringbuf-overflow 2>&1 | tee /root/ringbuf-overflow-2.9.9.F.8.txt
register_pipeline_observer_root
...
EBPF_GUARD_API="$CPU_API" EBPF_GUARD_TOKEN="$CPU_TOKEN" \
    bash ./run-all-attacks.sh --cpu-pressure-control 2>&1 | tee /root/cpu-pressure-2.9.9.F.8.txt
register_pipeline_observer_root
...
if [ "${SMOKE_ONLY:-0}" = "1" ]; then
    ...
    EBPF_GUARD_API="$CRED_API" EBPF_GUARD_TOKEN="$CRED_TOKEN" \
        bash ./run-all-attacks.sh --cred-proc-maps-control 2>&1 | tee /root/cred-proc-maps-2.9.9.F.8.txt
    register_pipeline_observer_root
...
if [ "${SMOKE_ONLY:-0}" = "1" ]; then
    ...
    echo "=== ПРЕДПРОГОН №2.9.9.F.8 ЗАВЕРШЁН $(date -u +%H:%M:%S) UTC ==="
...
export AGENT_START_FILE=/root/agent-start-2.9.9.F.8.txt
bash ./run-gate.sh 2>&1 | tee /root/gate-2.9.9.F.8.txt
GATE_RC=${PIPESTATUS[0]}
echo "гейт вернул $GATE_RC"
# Пункты 1-14 постановки №2.9.9.F.8 не подлежат толкованию — вытаскиваем их
...
acc_line=$(grep -E '^RUN-GATE: PASS=' /root/gate-2.9.9.F.8.txt | tail -1)
acc_skip=$(echo "$acc_line" | sed -E 's/.*SKIP=([0-9]+).*/\1/')
acc_verdict=$(grep -cE '^\[FAIL\]|\[0;31m\[FAIL\]' /root/gate-2.9.9.F.8.txt || true)
...
acc_skip_doc9=$(grep -cE '\[SKIP\].*5\.9\.9\.F\.7c' /root/gate-2.9.9.F.8.txt || true)
acc_skip_doc21=$(grep -cE '\[SKIP\].*stale_transitions=0' /root/gate-2.9.9.F.8.txt || true)
...
if [ "${acc_skip_eff}" = "0" ] && [ "$GATE_RC" -eq 0 ]; then
    ...
    grep -E '\[SKIP\]|\[FAIL\]' /root/gate-2.9.9.F.8.txt | sed 's/^/       /'
    grep -E 'чья ветка не исполнилась' /root/gate-2.9.9.F.8.txt | sed 's/^/       /'
fi
...
grep -E 'verdict="attack" за idle-час|алертов за idle-час|состав промоушенов по comm|промоушенов verdict=attack в журнале|промоушенов в журнале за окно idle-часа \(оба вердикта' /root/gate-2.9.9.F.8.txt | sed 's/^/       /'
...
grep -E 'по журналу \(5\.9\.9\.F\.5b\)' /root/gate-2.9.9.F.8.txt | sed 's/^/       /'
...
# вход.
grep -E 'ДОСТИГНУТО \(5\.9\.9\.F\.5b/5\.9\.9\.F\.5l\)' /root/gate-2.9.9.F.8.txt | sed 's/^/       /'
acc_idle_alerts=$(grep -oE 'алертов за idle-час: [0-9]+' /root/gate-2.9.9.F.8.txt | tail -1 | grep -oE '[0-9]+$')
acc_idle_att=$(grep -oE 'verdict="attack" за idle-час: [0-9]+ -> [0-9]+, дельта = [0-9-]+' /root/gate-2.9.9.F.8.txt | tail -1 | grep -oE '[0-9-]+$')
...
grep -E 'новые акторы idle-часа вне idle-actors|новых акторов 0|состав ВСЕХ новых алертов idle-часа' /root/gate-2.9.9.F.8.txt | sed 's/^/       /'
if grep -q 'новый(е) актор(ы) idle-часа вне idle-actors.txt' /root/gate-2.9.9.F.8.txt; then
...
acc_fwupd_stat=$(awk '
    ...
    END { printf "%d %d", n+0, alerts+0 }' /root/gate-2.9.9.F.8.txt 2>/dev/null || echo "0 0")
...
grep -E 'добавлено по фазам|наведено шагом преflight' /root/gate-2.9.9.F.8.txt | head -14 | sed 's/^/       /'
acc_induced=$(grep -oE 'наведено преflight-шагом=[0-9]+' /root/gate-2.9.9.F.8.txt | tail -1 | grep -oE '[0-9]+$')
acc_induced_ids=$(awk '/наведено шагом преflight.*вне измеряемых окон/{f=1; next} f && /^    \* /{sub(/^    \* /,""); print; next} f && !/^    \* /{exit}' /root/gate-2.9.9.F.8.txt)
...
acc_excl_obs=$(grep -oE 'excluded\{observer_tree\}[^:]*: [0-9]+' /root/gate-2.9.9.F.8.txt | tail -1 | grep -oE '[0-9]+$')
...
acc_drift_cut_line=$(grep -E 'drift_exec_from_system_bin[[:space:]]+стор' /root/gate-2.9.9.F.8.txt | tail -1)
...
acc_stale=$(grep -oE 'stale_transitions за прогон: [0-9]+' /root/gate-2.9.9.F.8.txt | tail -1 | grep -oE '[0-9]+$')
...
# только то, что величина 13 читается из /root/report-2.9.9.F.8.txt, и обе
...
acc "13." "критерий №68 (5.9.9.F.7b/№174) — в ОТЧЁТЕ, не в гейте: исход читается из /root/report-2.9.9.F.8.txt после шага [14/14]"
...
acc_daem_inc=$(grep -E 'доля по инцидентам \(после коалесцирования' /root/gate-2.9.9.F.8.txt | tail -1 | sed 's/^[[:space:]]*//')
acc_daem_alr=$(grep -E 'доля по алертам \(объём' /root/gate-2.9.9.F.8.txt | tail -1 | sed 's/^[[:space:]]*//')
...
acc_blind=$(grep -cE 'случай 3.*pid=|pid=[0-9]+ ppid=.* comm=.*смещение' /root/gate-2.9.9.F.8.txt || true)
acc_blind_line=$(grep -E 'дерево измерителя по случаю' /root/gate-2.9.9.F.8.txt | tail -1 | sed 's/^[[:space:]]*//')
...
echo "=== конец приёмки волны 5.9.9.F.8 (величины 1/4) + регресс волн 5.9.9.F.3…F.7 (вердикт гейта: $GATE_RC) ==="
echo ""
...
# №2.9.6…№2.9.8 тоже). REPORT_LABEL=2.9.9.F.8 чинит это для report-2.9.9.F.8.txt;
# если когда-нибудь появится собственный run-2.9.9.F.8-report.sh, ему тоже
# следует читать REPORT_LABEL, а не зашивать номер.
if [ -f "$SETUP/run-2.9.9.F.8-report.sh" ]; then
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.9.F.8.txt REPORT_LABEL=2.9.9.F.8 \
        bash $SETUP/run-2.9.9.F.8-report.sh 2>&1 | tee /root/report-2.9.9.F.8.txt
else
    echo "ВНИМАНИЕ: run-2.9.9.F.8-report.sh отсутствует — считаем отчётом (сверх гейта) фолбэком run-2.9.5-report.sh."
    echo "ВНИМАНИЕ: п.10 (idle-алерты на метках срезов) он посчитает, величины"
    echo "ВНИМАНИЕ: постановки №2.9.9.F.8 (пп.1-14) — нет; они только в гейте."
    IDLE_OUT=$IDLE_OUT AGENT_START_FILE=/root/agent-start-2.9.9.F.8.txt REPORT_LABEL=2.9.9.F.8 \
        bash $SETUP/run-2.9.5-report.sh 2>&1 | tee /root/report-2.9.9.F.8.txt
fi
...
drift_out=/root/drift-composition-2.9.9.F.8.txt
{
    echo "=== состав drift_exec_from_system_bin за аптайм, замер №2.9.9.F.8, $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
...
echo "  наблюдение сложено в $drift_out (собирается вместе с /root/report-2.9.9.F.8.txt)"

echo "=== ПАЙПЛАЙН №2.9.9.F.8 ЗАВЕРШЁН $(date -u +%H:%M:%S) UTC, гейт=$GATE_RC ==="
touch /root/PIPELINE-2.9.9.F.8-DONE
