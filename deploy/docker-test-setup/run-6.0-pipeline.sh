...
# --- ВОЛНА 6.0m, п.4: РАЗВЕДЕНИЕ СЕМАНТИКИ ОСТАНОВА -------------------------
#
# ЧТО БЫЛО. Любой провал любого шага-контроля звал die() и убивал прогон
# целиком. В окне атак это стоит уже отработавшего idle-часа: замеры 6.0f,
# 6.0f2 и 6.0j умерли так — все три на механике контроля, ни один на дефекте
# продукта, и каждый унёс с собой полностью снятые величины idle-часа,
# которые были уже в архиве и годились к разбору. Разбор №6.0j это и показал:
# гейт по архиву того прогона, запущенный постфактум, дал RUN-GATE: PASS.
#
# ГРАНИЦА. Право убить прогон имеет ровно то, что делает ПРОГОН НЕИЗМЕРИМЫМ:
#   * весь преflight ([0/14]) и всё, что стоит ДО idle-часа, — там смерть
#     стоит минуту, а не два часа, и это её единственное оправдание;
#   * загрязнение или недостоверность самого измерения (нет снимка, час
#     контаминирован, база не в enforcing к открытию часа, репетиция 6.0.19
#     красная) — величины после такого не значат ничего;
#   * отказ инфраструктуры замера (агент не поднялся, jq нет, гейт не найден).
# Провал ОТДЕЛЬНОГО контроля в окне атак таким не является: он оставляет свой
# критерий неизмеренным и не трогает ни одной чужой величины. Такой провал
# обязан записать вердикт и пустить прогон дальше — до гейта [13/14] и до
# приёмки, где его и судят поимённо.
#
# ЧЕМ ЭТО НЕ ЯВЛЯЕТСЯ. Это не смягчение критериев: soft_fail() не делает
# провал зелёным. Он делает его ИЗМЕРЕННЫМ — с именем, временем, причиной и
# строкой в приёмке, — вместо того чтобы обменивать его на потерю всего
# остального. Приёмка ниже читает $SOFT_FAIL_FILE и печатает НЕ ДОСТИГНУТО по
# каждому такому критерию отдельно.
SOFT_FAIL_FILE=/root/control-fails-6.0.txt
SOFT_FAIL_COUNT=0
soft_fail() {
    echo "=== КОНТРОЛЬ ПРОВАЛЕН, ПРОГОН ПРОДОЛЖАЕТСЯ (волна 6.0m): $* ==="
    SOFT_FAIL_COUNT=$((SOFT_FAIL_COUNT + 1))
    {
        ...
        printf '%s\n' "$*"
        echo "---"
    } >> "$SOFT_FAIL_FILE" 2>/dev/null || true
    return 0
}

# Источник истины о вердикте отдельного контроля — реестр drift-controls-6.0.txt,
# в который каждый шаг пишет свою строку FAIL ПЕРЕД остановом (и писал ещё до
# волны 6.0m — правка ничего здесь не изобретает, а начинает этот реестр
# читать). Проверка нужна ровно затем, чтобы строка «доказан живьём» не
# напечаталась после soft_fail: у 6.0.12 ветвей провала две, и обрамление
# каждой через if/else разошлось бы с реестром на первой же новой ветке.
_ctl_failed() { # $1=критерий -> 0, если контроль записал FAIL на этом прогоне
    grep -q "^$1 FAIL " /root/drift-controls-6.0.txt 2>/dev/null
}
...
grep -qE '_impact_critical_count container_escape_host_device\)' "$SETUP/wave7-controls.sh" \
    || die "преflight 6.0f/№200 (после переноса волной 6.0m — искать в wave7-controls.sh): контейнерный id container_escape_host_device не несёт СВОЕГО негативного контроля в этом пайплайне (греп якорен на полный id: префиксное совпадение с _from_host запрещено правкой 6.0j/п.5) — разведение container/host без негативного контроля есть заведение немого правила"
grep -qE '_hd_count container_escape_host_device_from_host warning' "$SETUP/wave7-controls.sh" \
    || die "преflight 6.0j/№211 (после переноса волной 6.0m — искать в wave7-controls.sh): хостовой id container_escape_host_device_from_host не несёт ПОЗИТИВНОГО контроля (6.0.18, шаг 6.0l) — правило warning-уровня, и негативный 6.0.13 (считает только критикалы) о нём не утверждает ничего. Ровно так оно и было заведено немым волной 6.0f: два FAIL гейта №6.0f2 («потеряно 1 типов детекта» и «1 правило немо без строки в silent-rules.txt») — это его цена, и они воспроизведутся на любом прогоне своей волны, потому что new-rules.txt вычитает правило только на архивах СТАРШЕ даты заведения"
...
# Волна 6.0m: шаг 6.0l вынесен в wave7-controls.sh вместе с 6.0h и 6.0k.
# Сторож не снят, а перенацелен: механика обязана существовать и после
# переноса, иначе волна 7 приняла бы пустой файл за исполненный контроль.
grep -qF 'echo "--- 6.0l:' "$SETUP/wave7-controls.sh" \
    || die "преflight 6.0j/№211 (после переноса волной 6.0m): шага с меткой 6.0l нет в wave7-controls.sh — критерий 6.0.18 негде исполнить, и перенос потерял механику вместо того, чтобы её сохранить"
grep -qF 'echo "--- 6.0h:' "$SETUP/wave7-controls.sh" \
    || die "преflight 6.0m: шага 6.0h нет в wave7-controls.sh — контроли №200/№205 (6.0.13/6.0.14/6.0.15) потеряны переносом"
grep -qF 'echo "--- 6.0k:' "$SETUP/wave7-controls.sh" \
    || die "преflight 6.0m: шага 6.0k нет в wave7-controls.sh — контроли №210 (6.0.16/6.0.17) потеряны переносом"
grep -qF '№215' "$SETUP/wave7-controls.sh" \
    || die "преflight 6.0m: в wave7-controls.sh нет записи о находке №215 (у 6.0.14 нет сторожа результата) — волна 7 запустила бы тот же приборный ноль, на котором умер замер №6.0j"
...
# --- преflight ВОЛНЫ 6.0m: три инварианта, которыми волна и держится --------
#
# П.5/№215. РЕПЕТИЦИЯ ВПРАВЕ СУДИТЬ ТОЛЬКО ПО РЕЗУЛЬТАТУ, НЕ ПО КОДУ ВОЗВРАТА.
# Замер №6.0j умер на 6.0.14 приборным нулём: `dd if=/dev/zero of=/dev/vda1
# bs=512 count=0` не делает ни одного write(2) (FILE_OP_WRITE ставится только
# в хуке sys_enter_write, bpf/fileaccess.bpf.c), правило требует op=write —
# события не существовало вовсе. Репетиция 6.0.19 этот случай пропустила ПО
# ПОСТРОЕНИЮ: единственный её блок, судивший по `rc`, печатал «механика цела»,
# потому что `dd count=0` возвращает 0. Сторож запрещает возврат этой
# конструкции: внутри области репетиции не должно быть ни одного сравнения
# кода возврата, и сентинельных проверок обязано быть не меньше, чем
# утверждений «механика цела».
_m_reh=$(awk '/^# 6\.0\.19: НАЧАЛО РЕПЕТИЦИИ/,/^# 6\.0\.19: КОНЕЦ РЕПЕТИЦИИ/' "$SETUP/run-6.0-pipeline.sh")
[ -n "$_m_reh" ] \
    || die "преflight 6.0m/№215: границы области репетиции (маркеры «6.0.19: НАЧАЛО/КОНЕЦ РЕПЕТИЦИИ») не найдены — сторож, ради которого волна 6.0m и заведена, проверял бы пустую строку и был бы ложно-зелёным"
_m_rc=$(printf '%s\n' "$_m_reh" | grep -cE '_rc(:-|\}| )' || true)
[ "${_m_rc:-0}" -eq 0 ] \
    || die "преflight 6.0m/№215: в области репетиции 6.0.19 снова появилось суждение по коду возврата ($_m_rc строк с _rc). Репетиция обязана утверждать СЕНТИНЕЛ — файл, созданный самим исполненным примитивом, — и никогда код возврата вызова: «dd count=0» возвращает 0, не сделав ни одного write(2), и ровно так репетиция объявила механику целой перед контролем 6.0.14, на котором умер замер №6.0j"
# Счёт по ВХОЖДЕНИЯМ, а не по строкам: форма «[ -e X ] && _reh_note»
# живёт в одной строке, и построчный счётчик её не видел — сторож был бы
# слепым ровно к самой частой записи утверждения.
_m_note=$(printf '%s\n' "$_m_reh" | grep -o '_reh_note "' | wc -l | tr -d ' ')
_m_sent=$(printf '%s\n' "$_m_reh" | grep -cE '\[ -e "\$|_reh_exec_probe ' || true)
[ "${_m_sent:-0}" -ge "${_m_note:-99}" ] \
    || die "преflight 6.0m/№215: в области репетиции 6.0.19 утверждений «механика цела» ($_m_note) больше, чем сентинельных проверок ($_m_sent) — значит хотя бы одно из них выносится не по наблюдаемому результату действия. Это и есть класс находок №176/№215: контроль печатает вердикт о том, чего не измерял"

# П.3. КОНТРОЛИ ЧУЖИХ НАХОДОК НЕ ЖИВУТ В ПАЙПЛАЙНЕ 6.0. Шаги 6.0h/6.0k/6.0l
# вынесены в wave7-controls.sh; их возвращение сюда вернуло бы общую точку
# отказа посреди двухчасового прогона, из-за которой умерли 6.0f, 6.0f2 и 6.0j.
for _m_step in 6.0h 6.0k 6.0l; do
    grep -q "^echo \"--- $_m_step:" "$SETUP/run-6.0-pipeline.sh" \
        && die "преflight 6.0m/п.3: шаг $_m_step снова заведён в пайплайне 6.0 — он вынесен в wave7-controls.sh как контроль ЧУЖОЙ находки (№200/№205/№210/№211), к базовой линии дрейфа отношения не имеющий. Возврат сюда воспроизводит конструкцию, которой стоили три аборта подряд: провал болт-он контроля убивает весь прогон ПОСЛЕ отработавшего idle-часа"
done
# Якорь — ВЫЗОВ в начале строки, а не текст: без него греп ловил бы
# собственную строку этого сторожа и падал на пустом месте.
grep -qE '^[[:space:]]*dd if=/dev/zero .*count=0' "$SETUP/run-6.0-pipeline.sh" \
    && die "преflight 6.0m/№215: в пайплайне 6.0 снова появился «dd … count=0» как вход контроля — это вход, не делающий ни одного write(2), то есть позитивный контроль на пустом месте. Смотреть wave7-controls.sh, где записана цена честного входа (свой узел mknod на loop-устройство, а не /dev/vda1)"

# П.4. СЕМАНТИКА ОСТАНОВА РАЗВЕДЕНА. Провал ОТДЕЛЬНОГО контроля в окне атак
# обязан звать soft_fail() (вердикт + прогон идёт до гейта), а не die()
# (смерть прогона). Проверяется машинно, а не обещанием в комментарии.
_m_hard=$(grep -cE '^\s*die "6\.0\.(3|6|7|11|12) ПРОВАЛЕН' "$SETUP/run-6.0-pipeline.sh" || true)
[ "${_m_hard:-0}" -eq 0 ] \
    || die "преflight 6.0m/п.4: $_m_hard контролей окна атак снова зовут die() на своём провале. Право убить прогон имеет только то, что делает его НЕИЗМЕРИМЫМ (преflight, загрязнение часа, отказ инфраструктуры); провал одного контроля оставляет неизмеренным один критерий и обязан звать soft_fail(). Разбор №6.0j: гейт по архиву умершего прогона дал RUN-GATE: PASS — то есть die() выбросил полностью годный замер"
_m_soft=$(grep -cE '^\s*soft_fail "6\.0\.(3|6|7|11|12) ПРОВАЛЕН' "$SETUP/run-6.0-pipeline.sh" || true)
[ "${_m_soft:-0}" -ge 5 ] \
    || die "преflight 6.0m/п.4: вызовов soft_fail() на провалах контролей окна атак только $_m_soft, ожидалось не меньше 5 (6.0.3/6.0.6/6.0.7/6.0.11/6.0.12) — часть провалов перестала записывать вердикт вовсе, и критерий стал бы молча зелёным"
echo "  ок (волна 6.0m): репетиция судит по сентинелам ($_m_sent проверок на $_m_note утверждений, суждений по rc: 0); шаги 6.0h/6.0k/6.0l вынесены; провалов контролей через die(): 0, через soft_fail(): $_m_soft"
...
echo "--- 6.0.19: репетиция механики ТРЁХ шагов-контролей ДО idle-часа (№209, п.3 волны 6.0j; список сокращён волной 6.0m) ---"
# 6.0.19: НАЧАЛО РЕПЕТИЦИИ (граница для сторожа «преflight 6.0m/№215» — не убирать)
# Зачем. SMOKE_ONLY=1 покрывает шаги [1/14]…[6/14]; шаги контролей
# 6.0c/6.0e/6.0g стоят ПОСЛЕ окна атак и не репетируются
...
# 0/4. Все ТРИ шага-контроля обязаны существовать в этом файле. Репетиция
# несуществующего шага — ложно-зелёная: она напечатала бы «механика цела» про
# то, чего нет. Список закрыт и совпадает с постановкой волны 6.0j, из
# которой волна 6.0m вычла 6.0h/6.0k/6.0l вместе с самими шагами (они
# вынесены в wave7-controls.sh, их репетиция принадлежит волне 7).
for _reh_step in 6.0c 6.0e 6.0g; do
    grep -q "^echo \"--- $_reh_step:" "$SETUP/run-6.0-pipeline.sh" \
        || _reh_bad "$_reh_step" "шага с меткой $_reh_step нет в run-6.0-pipeline.sh — репетировать нечего, а приёмка волны требует всех трёх (список сокращён волной 6.0m: 6.0h/6.0k/6.0l вынесены в wave7-controls.sh вместе с критериями 6.0.13…6.0.18)"
done
...
# 1/4 (6.0c) и 3/4 (6.0g) — один и тот же примитив в трёх каталогах: обычный
...
# 2/4. Событие доезжает и proc.args в алерте НЕПУСТ. Это и есть находка №209
...
# 4/4 (6.0e). Примитив побега целиком: unshare(2) + mount(MS_REC|MS_PRIVATE)
...
# Блоки 5/7 (6.0h/6.0l) и 6/7 (6.0k) УДАЛЕНЫ ВОЛНОЙ 6.0m вместе со своими
# шагами: те вынесены в $SETUP/wave7-controls.sh. Репетировать здесь нечего —
# репетиция шага, которого в этом пайплайне нет, ложно-зелёная по построению.
#
# ЧТО ИЗ НИХ ВАЖНО ЗАПОМНИТЬ (п.5 волны 6.0m, находка №215). Удалённый блок
# 5/7 был единственным во всей репетиции, который выносил «механика цела» ПО
# КОДУ ВОЗВРАТА: он печатал «dumpe2fs rc=0, dd count=0 rc=?» и не проверял
# НИЧЕГО о результате. Ровно поэтому он пропустил 6.0.14: `dd count=0`
# возвращает 0, не сделав ни одного write(2), и репетиция объявляла механику
# целой ровно перед тем контролем, который потом умирал приборным нулём.
# Все прочие блоки (6.0c, 6.0e, 6.0g, а также удалённый 6.0k) утверждают
# СЕНТИНЕЛ — файл, созданный самим исполненным примитивом. Это и есть
# граница: репетиция вправе печатать «механика цела» только по наблюдаемому
# результату действия, никогда — по коду возврата вызова. Машинный сторож
# этого правила стоит в преflight'е (см. «преflight 6.0m/№215» выше).
rm -rf "$_reh_c_dir" "$_reh_g_seed" "$_reh_g_novel"
rm -f "/tmp/reh-ran-$_reh_c_comm" "/tmp/reh-ran-$_reh_g_comm"
fi
# 6.0.19: КОНЕЦ РЕПЕТИЦИИ (граница для сторожа «преflight 6.0m/№215» — не убирать)
...
echo "6.0.19 ДОСТИГНУТО $(date -u +%H:%M:%S) UTC: механика всех ТРЁХ шагов-контролей (6.0c/6.0e/6.0g) отрепетирована до idle-часа по СЕНТИНЕЛАМ, а не по кодам возврата (вердикты контролей этим НЕ предвосхищены; список сокращён волной 6.0m — 6.0h/6.0k/6.0l вынесены в wave7-controls.sh)"
...
if [ "${SMOKE_ONLY:-0}" = "1" ]; then
    echo "=== SMOKE_ONLY=1: сборка, SMOKE, РЕПЕТИЦИЯ МЕХАНИКИ ТРЁХ ШАГОВ-КОНТРОЛЕЙ (6.0.19, список сокращён волной 6.0m), оба контроля DNS, контроль счётности, переполнение кольца, наведённое CPU-давление (5.9.9.F.2b), ТРИ ПАРЫ КОНТРОЛЕЙ ПРОДУКТОВЫХ ПРАВОК (5.9.9.F.3a/3b/3c), позитивный контроль cred_proc_maps и пары 5.9.9.F.1b/5.9.9.F.1c пройдены ==="
...
if [ "$((${_drift_pc2_after:-0} - ${_drift_pc2_before:-0}))" -lt 1 ]; then
    ...
    soft_fail "6.0.3 ПРОВАЛЕН: exec нового бинаря $DRIFT_PC2_BIN тем же comm=drift-pc, что уже enforcing с шага [10/14], не дал ни одного нового срабатывания drift_exec_from_system_bin (было ${_drift_pc2_before:-0}, стало ${_drift_pc2_after:-0}) — либо базовая линия свернула правило в постоянную тишину для этого workload'а (регрессия сигнатуры syscall-события), либо посев [10/14] и контроль здесь разошлись комм-именами"
fi
# Волна 6.0m: строка «доказан живьём» печатается только если контроль
# выше не записал FAIL. До этой правки провал звал die() и до сюда не
# доходил; теперь он зовёт soft_fail() и прогон идёт дальше — без
# обрамления те же две строки объявили бы проваленный контроль
# доказанным, что хуже смерти прогона.
if _ctl_failed 6.0.3; then
    echo "6.0.3 НЕ ДОКАЗАН на этом прогоне: вердикт записан в $SOFT_FAIL_FILE, прогон продолжается до гейта (волна 6.0m)"
else
...
# обрамления те же две строки объявили бы проваленный контроль
# доказанным, что хуже смерти прогона.
if _ctl_failed 6.0.6; then
    echo "6.0.6 НЕ ДОКАЗАН на этом прогоне: вердикт записан в $SOFT_FAIL_FILE, прогон продолжается до гейта (волна 6.0m)"
else
...
# обрамления те же две строки объявили бы проваленный контроль
# доказанным, что хуже смерти прогона.
if _ctl_failed 6.0.7; then
    echo "6.0.7 НЕ ДОКАЗАН на этом прогоне: вердикт записан в $SOFT_FAIL_FILE, прогон продолжается до гейта (волна 6.0m)"
else
...
# обрамления те же две строки объявили бы проваленный контроль
# доказанным, что хуже смерти прогона.
if _ctl_failed 6.0.11; then
    echo "6.0.11 НЕ ДОКАЗАН на этом прогоне: вердикт записан в $SOFT_FAIL_FILE, прогон продолжается до гейта (волна 6.0m)"
else
...
# обрамления те же две строки объявили бы проваленный контроль
# доказанным, что хуже смерти прогона.
if _ctl_failed 6.0.12; then
    echo "6.0.12 НЕ ДОКАЗАН на этом прогоне: вердикт записан в $SOFT_FAIL_FILE, прогон продолжается до гейта (волна 6.0m)"
else
...
# --- ВОЛНА 6.0m: ШАГИ 6.0h / 6.0k / 6.0l ВЫНЕСЕНЫ ОТСЮДА ---------------------
#
# Здесь стояли три шага-контроля ЧУЖИХ находок — №200/№205 (6.0h: критерии
# 6.0.13/6.0.14/6.0.15), №210 (6.0k: 6.0.16/6.0.17) и №211 (6.0l: 6.0.18).
# Ни один из них не про базовую линию дрейфа, ради которой заведён пункт 6.0.
# Они перенесены построчно, без правок логики, в $SETUP/wave7-controls.sh и
# принимаются волной 7 вместе с находкой №215 (у 6.0.14 нет сторожа
# результата — на нём умер замер №6.0j).
#
# ПОЧЕМУ ПЕРЕНОС, А НЕ УДАЛЕНИЕ И НЕ «ОСТАВИТЬ КАК БЫЛО». Пункт 6.0 стартовал
# с четырёх критериев и к прогону №6.0j нёс девятнадцать: 4 -> 5 -> 9 -> 15 ->
# 19. Прирост — находки, обнаруженные ВО ВРЕМЯ измерения 6.0 и прицепленные к
# тому же пайплайну. Правило «правка правила без контроля есть заведение
# немого правила» верное, и контроли обязаны существовать. Неверно было
# МЕСТО: шаги стояли в окне атак, ПОСЛЕ отработавшего idle-часа, и каждый
# die'ил весь прогон. Три аборта подряд (6.0f, 6.0f2, 6.0j) — все на механике
# контроля, ни один на дефекте продукта; каждый стоил двух часов стенда и
# обнулял уже собранные величины 6.0. Удалить их значило бы завести три немых
# правила разом; оставить — сохранить общую точку отказа посреди двухчасового
# прогона. Перенос снимает пять критериев (6.0.14…6.0.18) из
# WAVE_CRITERIA_IDS и три шага из окна атак, не теряя ни строки механики.
# ----------------------------------------------------------------------------
...
# 16.3 — печатается циклом ниже (волна 6.0m), по реестру drift-controls-6.0.txt:
# «приёмка дошла до строки» больше не доказывает ДОСТИГНУТО, шаг зовёт soft_fail().
...
# 16.6 — печатается циклом ниже (волна 6.0m), по реестру drift-controls-6.0.txt.
# 16.7 — печатается циклом ниже (волна 6.0m): прежняя формулировка «шаг
# die'ит сам, значит дошли — ДОСТИГНУТО» перестала быть верной, шаг зовёт soft_fail().
...
# 16.11-16.12 — волна 6.0f, два контроля механизма (а). Волна 6.0m изменила
# здесь ровно одно: раньше эти шаги `die`'или прогон на провале, и «дошло до
# этой строки» само по себе означало ДОСТИГНУТО. Теперь они зовут soft_fail()
# и прогон доходит сюда В ЛЮБОМ случае, поэтому исход читается из реестра,
# а не из факта, что приёмка исполняется.
#
# 16.13-16.18 УБРАНЫ: критерии 6.0.13…6.0.18 вынесены волной 6.0m в
# wave7-controls.sh вместе со своими шагами (6.0h/6.0k/6.0l) и принимаются
# волной 7. Ни один из них не про базовую линию дрейфа: №200/№205 — правила
# импакта и побега, №210 — актуальность proc.args, №211 — хостовой id.
for _acc_c in 6.0.3 6.0.6 6.0.7 6.0.11 6.0.12; do
    _acc_n=$(printf '16.%s' "${_acc_c#6.0.}")
    if _ctl_failed "$_acc_c"; then
        acc "$_acc_n" "$_acc_c НЕ ДОСТИГНУТО — контроль записал FAIL, прогон продолжен волной 6.0m; причина целиком в $SOFT_FAIL_FILE и в строке 'КОНТРОЛЬ ПРОВАЛЕН' этого лога"
    elif grep -q "^$_acc_c PASS " /root/drift-controls-6.0.txt 2>/dev/null; then
        acc "$_acc_n" "$_acc_c ДОСТИГНУТО — $(grep "^$_acc_c PASS " /root/drift-controls-6.0.txt | tail -1)"
    else
        acc "$_acc_n" "$_acc_c НЕ ИЗМЕРЕН — в drift-controls-6.0.txt нет ни PASS, ни FAIL: шаг не исполнялся вовсе (прогон умер раньше окна атак либо шаг потерян правкой)"
    fi
done
if [ "${SOFT_FAIL_COUNT:-0}" -gt 0 ]; then
    acc "  " "проваленных контролей за прогон (soft_fail, волна 6.0m): $SOFT_FAIL_COUNT — прогон доведён до гейта СОЗНАТЕЛЬНО, чтобы величины idle-часа и вердикт гейта не были потеряны вместе с одним контролем; каждый провал назван поимённо выше"
else
    acc "  " "проваленных контролей за прогон (soft_fail, волна 6.0m): 0"
fi
