...
# перенумеровала метки 18/21 -> 18/22 — сторож валил преflight на РАСХОЖДЕНИИ
...
grep -qF "replay-gate.sh на ТРИНАДЦАТИ архивах, ${_replay_denoms} реплей" "$SETUP/run-6.0-pipeline.sh" \
...
echo "=== [1/14] ЖЁСТКИЙ СТОП №1: replay-gate.sh на ТРИНАДЦАТИ архивах, 22 реплей (5.9.7c…6.0f) ==="
...
# 6.0g (предпрогон 01.09): волна 6.0f завела реплеи 19/22..22/22 (6.0.5,
...
echo "  архив 6.0 (реплей 19/22): ${C60_DIR:-НЕ НАЙДЕН}"
echo "  архив 6.0b (реплеи 19/22, 20/22, 21/22, 22/22): ${C60B_DIR:-НЕ НАЙДЕН}"
if [ -z "$C295_DIR" ] || [ -z "$C296_DIR" ] || [ -z "$C297_DIR" ] || [ -z "$C298_DIR" ] || [ -z "$C299_DIR" ] || [ -z "$C299F_DIR" ] || [ -z "$C299F1_DIR" ] || [ -z "$C299F2_DIR" ] || [ -z "$C294_DIR" ] || [ -z "$C299F6_DIR" ] || [ -z "$C299F7_DIR" ] || [ -z "$C60_DIR" ] || [ -z "$C60B_DIR" ]; then
    die "архивы collect-2.9.5…collect-2.9.9.F.2, collect-2.9.4, collect-2.9.9.F.6, collect-2.9.9.F.7, collect-6.0, collect-6.0b найдены не все на стенде. Скопировать (например в /root/) либо задать REPLAY_C295_DIR…REPLAY_C299F2_DIR/REPLAY_C294_DIR/REPLAY_C299F6_DIR/REPLAY_C299F7_DIR/REPLAY_C60_DIR/REPLAY_C60B_DIR. Пропуск реплея — это находка №85, повторённая тринадцатый раз"
fi
...
# Реплеи 19/22..22/22 читают у архивов 6.0/6.0b не attacks/ вообще, а
...
# ЗДЕСЬ, до вызова, тем же приёмом, что 15/22..18/22.
[ -s "$C60_DIR/run-6.0-pipeline.log" ] \
    || die "реплей 19/22: $C60_DIR/run-6.0-pipeline.log отсутствует — сторона «НЕ ДОСТИГНУТО» решающего правила 6.0.5 (6.0.3 провален, №187) непроверяема"
[ -s "$C60B_DIR/run-6.0-pipeline.log" ] \
    || die "реплей 19/22: $C60B_DIR/run-6.0-pipeline.log отсутствует — сторона «ДОСТИГНУТО» решающего правила 6.0.5 непроверяема, и первый исход не доказывал бы ничего"
for f in idle/metrics-start.txt idle/metrics-end.txt gate-6.0.txt; do
    [ -s "$C60B_DIR/$f" ] \
        || die "реплей 20/22: $C60B_DIR/$f отсутствует — величина 6.0.1 (объём прямой дельтой двух метрик, №199) не с чем сверять"
done
[ -s "$C299F7_DIR/gate-2.9.9.F.7.txt" ] \
    || die "реплей 20/22: $C299F7_DIR/gate-2.9.9.F.7.txt отсутствует — сторона «НЕ ДОСТИГНУТО 577/ч» величины 6.0.1 непроверяема"
ls "$C60B_DIR"/attacks/baseline-state-*.json >/dev/null 2>&1 \
    || die "реплеи 21/22 и 22/22: в $C60B_DIR/attacks нет baseline-state-*.json — офлайн-гейт по архиву не запустить, ни ветка drift-suppression (5.9.7e/№194), ни 6.0.10 (контаминация idle-часа логином, №202) не получат входа"
for f in idle/metrics-start.txt idle/metrics-end.txt idle/alerts-start.json idle/alerts-end.json; do
    [ -s "$C60B_DIR/$f" ] \
        || die "реплей 22/22: $C60B_DIR/$f отсутствует — обе стороны 6.0.10 (чистый и контаминированный idle-час) остались бы без входа"
done
bash $SETUP/attacks/replay-gate.sh "$C295_DIR" "$C296_DIR" "$C297_DIR" "$C298_DIR" "$C299_DIR" "$C299F_DIR" "$C299F1_DIR" "$C299F2_DIR" "$C294_DIR" "$C299F6_DIR" "$C299F7_DIR" "$C60_DIR" "$C60B_DIR" 2>&1 | tee /root/replay-6.0.txt
