...
# должен читать. Флаг-страж от бесконечной рекурсии — W626_LOG_CAPTURED.
if [ -z "${W626_LOG_CAPTURED:-}" ]; then
    export W626_LOG_CAPTURED=1
    ...
    _w626_rcf=$(mktemp /tmp/.w626-rc.XXXXXX)
    { bash "$0" "$@" 2>&1; echo "$?" > "$_w626_rcf"; } | tee "$OUT"
    _w626_rc=$(cat "$_w626_rcf" 2>/dev/null || echo 1)
    rm -f "$_w626_rcf"
    exit "${_w626_rc:-1}"
fi
...
W626_FORCE_NODE_EVENT="${FORCE_NODE_EVENT:-1}" \
...
if [ -s "$OUT" ]; then
    if command -v iconv >/dev/null 2>&1 && iconv -f UTF-8 -t UTF-8 "$OUT" >/dev/null 2>&1; then
        echo "СТОРОЖ №309: run-6.2.6.log ВАЛИДЕН В UTF-8 — офлайн-разбор (grep/sed без -a) безопасен"
    else
        echo "СТОРОЖ №309: run-6.2.6.log НЕВАЛИДЕН В UTF-8 (либо iconv недоступен) — офлайн-разбор ОБЯЗАН использовать grep -a, иначе BSD grep молча ничего не найдёт"
    fi
fi
