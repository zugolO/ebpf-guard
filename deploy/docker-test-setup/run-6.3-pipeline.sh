...
_nd63w_dedup=0; _nd63w_emitted=0; _nd63w_ratelimit=0; _nd63w_anomaly=0
if [ -s "$_nd63w" ]; then
    while read -r _nd63w_o _nd63w_c; do
        case "$_nd63w_o" in
            ...
            anomaly_detection) _nd63w_anomaly=$_nd63w_c ;;
        esac
    done < <(awk "$_nd_jawk"'{ o = jstr($0, "outcome"); c = jnum($0, "count"); if (o != "" && c > 0) s[o] += c }
         END { for (k in s) printf "%s %d\n", k, s[k] }' "$_nd63w" 2>/dev/null)
fi
_nd63w_total=$(( _nd63w_dedup + _nd63w_emitted + _nd63w_ratelimit + _nd63w_anomaly ))
...
if [ "$_r63_nd_want" != "on" ]; then
    ...
    echo "OK: 6.3.9.2 ИЗМЕРЕНО (порог не назначается, правило 5.9.6): разрез цены по четырём слоям подавления покрывает 100% величины окна — dedup ${_nd63w_dedup}, emitted ${_nd63w_emitted}, rate_limit ${_nd63w_ratelimit}, anomaly_detection ${_nd63w_anomaly} (сумма ${_nd63w_total})"
