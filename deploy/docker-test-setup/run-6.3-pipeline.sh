...
if curl -s --max-time 30 -H "Authorization: Bearer $_r63_token" "$_r63_api/metrics" > /root/metrics-prologue-start-6.3.txt 2>/dev/null \
    ...
    _r63_burst=$(awk '/^ebpf_guard_events_dropped_total\{/ && !/reason="path_denylist"/ && $NF+0 > 0 { printf "%s=%d ", $1, $NF }
                      /^ebpf_guard_event_queue_dropped_total/ && $NF+0 > 0 { printf "%s=%d ", $1, $NF }' /root/metrics-prologue-start-6.3.txt)
...
_w63r_bl_t0=$(cat /root/agent-start-6.3.epoch 2>/dev/null)
...
if curl -s --max-time 30 -H "Authorization: Bearer $_r63_token" "$_r63_api/metrics" > "$ART/metrics-run-end.txt" 2>/dev/null \
    ...
    awk 'FNR==NR { if ($1 ~ /^ebpf_guard_(events|event_queue)_dropped/) v0[$1] = $NF; next }
        ...
        /root/metrics-prologue-start-6.3.txt "$ART/metrics-run-end.txt"
else
