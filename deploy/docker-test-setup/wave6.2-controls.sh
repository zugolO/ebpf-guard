...
# ПОРЯДОК: подача входа (6.2.1/6.2.2) идёт ПОСЛЕ тихого окна, а не до него.
# Причина — память control-after-attacks-hits-filled-limiter: позитивный
# контроль читает SA-токен десятками, лимитер правила (10/60с) после этого
# частично выбран, и окно объёма, открытое следом, показало бы величину
# лимитера вместо фона ноды. Окно меряется по чистому прибору, вход подаётся
# после.
# ─────────────────────────────────────────────────────────────────────────────
