...
if [ "$W621_INSTRUMENTED" -eq 1 ]; then
    ...
    if [ -z "$_w621_target8" ]; then
        ...
        # ИМЯ КОПИИ — РОВНО ИМЯ ДЕМОНА, и это не косметика (смок-прогон
        # 05.09.2026, дефект механики этого же контроля).
        #
        # comm ядро берёт из БАЗОВОГО ИМЕНИ ОБРАЗА в execve, а `exec -a`
        # подменяет только argv[0] и comm не трогает (та же механика, что в
        # памяти exec-a-argv0-spoof-kills-proc-args). Первая редакция копировала
        # cat в /tmp/w621-k3s-server и спрашивала стор про comm=="k3s-server":
        # фактический comm был "w621-k3s-server", запрос не находил ничего, и
        # контроль напечатал ПРОВАЛ «исключение следует за именем» — при том
        # что оба обязательных правила поднялись (проверено по стору: 3 алерта
        # от comm=w621-k3s-server). Ложный провал в сторону обвинения продукта.
        #
        # Хуже того, при таком имени контроль не проверял бы слой 2 и в случае
        # успеха: comm "w621-k3s-server" не совпадает со списком исключения
        # вовсе, правило сработало бы независимо от exe_path. Чтобы подделка
        # действительно надевала имя демона, копия обязана НАЗЫВАТЬСЯ k3s-server
        # — тогда исключение отсекается ровно образом (/tmp/), а не именем.
        rm -rf /tmp/w621-bypass 2>/dev/null
        mkdir -p /tmp/w621-bypass 2>/dev/null
        cp /bin/cat /tmp/w621-bypass/k3s-server 2>/dev/null
        chmod 0755 /tmp/w621-bypass/k3s-server 2>/dev/null
        ...
        #
        # PID подделки берётся ЯВНО и по нему же читается стор. Отбор по одному
        # comm здесь больше не годится: теперь она носит имя НАСТОЯЩЕГО демона
        # ноды, у которого в том же окне есть собственные алерты (на смоке — 19
        # от comm=k3s-server), и «нашёл алерт с этим comm» означало бы ПРОПУСК
        # обхода вместо его доказательства. Подстановка `( exec … ) &` заменяет
        # подоболочку образом, поэтому $! — это pid самой подделки.
        ( exec -a k3s-server /tmp/w621-bypass/k3s-server "$_w621_target8" ) > /tmp/w621-bypass/out 2>/dev/null &
        _w621_p8=$!
        wait "$_w621_p8" 2>/dev/null
        _w621_b8=$(wc -c < /tmp/w621-bypass/out 2>/dev/null | tr -d ' ')
        _w621_h8=0; _w621_w8=0
        while [ "$_w621_w8" -lt "$W621_POS_TIMEOUT" ]; do
            sleep "$W621_SETTLE"; _w621_w8=$(( _w621_w8 + W621_SETTLE ))
            _w621_h8=$(_w621_alerts | jq --argjson t "$_w621_t8" --argjson p "${_w621_p8:-0}" --arg ids "$W621_HOST_RULES" \
        ...
        _w621_c8=$(_w621_alerts | jq -r --argjson p "${_w621_p8:-0}" '[.[]|select(.pid == $p)][0].comm // "нет алертов от этого pid"' 2>/dev/null)
        echo "  сторож результата подделки: прочитано байт = $_w621_b8 (цель $_w621_target8)"
        echo "  подделка: pid=$_w621_p8, образ /tmp/w621-bypass/k3s-server, comm в сторе=$_w621_c8 (обязан быть k3s-server — иначе контроль проверяет не то)"
        echo "  алертов от подделки (по её pid): $_w621_h8 из обязательных ($W621_HOST_RULES)"
        if [ "${_w621_b8:-0}" -lt 1 ]; then
        ...
        elif [ "$_w621_c8" != "k3s-server" ] && [ "${_w621_h8:-0}" -lt 1 ]; then
            die "6.2.1.8 НЕИЗМЕРИМ (половина «отказ обхода»): подделка не носит имени демона — стор знает её как «$_w621_c8», а не k3s-server. При таком comm исключение node-host-daemon не применилось бы В ЛЮБОМ СЛУЧАЕ, и что бы контроль ни напечатал, слой 2 (образ против имени) он не проверял. Чинить контроль: имя КОПИИ обязано быть k3s-server, `exec -a` меняет только argv[0]"
            ...
            pass "6.2.1.8 ДОСТИГНУТО (половина «отказ обхода»): подделка носила имя демона (comm=$_w621_c8), но не унаследовала его тишину — поднято $_w621_h8 обязательных правил, исключение отсечено ОБРАЗОМ"
        fi
        rm -rf /tmp/w621-bypass 2>/dev/null
    fi
else
