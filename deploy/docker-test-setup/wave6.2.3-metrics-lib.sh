#!/usr/bin/env bash
# wave6.2.3-metrics-lib.sh — измерительная половина волны 6.2.3. Форк
# wave6.2.2-metrics-lib.sh: функции w623_ratelimited_by_rule/w623_volume_by_rule
...
#
#   w623_dedup_dropped_by_rule  — четвёртый слой (дедуп), по метрике;
#   w623_value_v_by_rule        — величина (в) поимённо: (а)-разбивка + срез
#                                  лимитера + срез дедупа на каждый rule_id,
#                                  ранжировано по СУММЕ, а не по остатку
#                                  после лимитера (это и есть критерий
#                                  6.2.3.2, отличие от 6.2.2.3);
#   w623_value_v_total           — та же сумма, одним числом, для сверки с
#                                  прямой дельтой в контролях.
#
# Файл самопроверяем: запуск
#
#   ./wave6.2.3-metrics-lib.sh --self-test ../../server-logs/collect-6.2.2-run1
#   ./wave6.2.3-metrics-lib.sh --self-test ../../server-logs/collect-6.2.2
#
...
# ---------------------------------------------------------------------------
w623_ratelimited_by_rule() { # $1=срез-начало $2=срез-конец
    awk '
        function rid(line,   m) {
...
# ---------------------------------------------------------------------------
w623_volume_by_rule() { # $1=срез-начало $2=срез-конец
    awk '
        function rid(line,   m) {
...
w623_volume_delta() { # $1=срез-начало $2=срез-конец
    awk '
        /^ebpf_guard_alerts_total[{ ]/ || /^ebpf_guard_alerts_filtered_total[{ ]/ {
...
# МЕТРИКЕ, симметрично w623_ratelimited_by_rule.
# ---------------------------------------------------------------------------
w623_dedup_dropped_by_rule() { # $1=срез-начало $2=срез-конец
    awk '
        function rid(line,   m) {
            if (match(line, /rule_id="[^"]*"/)) return substr(line, RSTART + 9, RLENGTH - 10)
            return ""
...
# ---------------------------------------------------------------------------
# №248 + РЕШЕНИЕ 1: величина (в) поимённо — критерий 6.2.3.2.
#
...
# ---------------------------------------------------------------------------
w623_value_v_by_rule() { # $1=срез-начало $2=срез-конец
    {
        w623_volume_by_rule "$1" "$2"
        w623_ratelimited_by_rule "$1" "$2" | awk '$2 != "RESET"'
        w623_dedup_dropped_by_rule "$1" "$2" | awk '$2 != "RESET"'
    } | awk '{ s[$1] += $2 } END { for (r in s) printf "%s %d\n", r, s[r] }' | sort -k2 -rn
}

# Прямая сумма величины (в) — для сверки разбивки с самой собой в контролях.
w623_value_v_total() { # $1=срез-начало $2=срез-конец
    local vol rl dd
    vol=$(w623_volume_delta "$1" "$2")
    rl=$(w623_ratelimited_by_rule "$1" "$2" | awk '$2 != "RESET" {s+=$2} END{printf "%d", s+0}')
    dd=$(w623_dedup_dropped_by_rule "$1" "$2" | awk '$2 != "RESET" {s+=$2} END{printf "%d", s+0}')
    printf "%d" "$(( vol + rl + dd ))"
}
...
# ---------------------------------------------------------------------------
w623_self_test() { # $1=каталог архива (…/collect-6.2.2-run1 или …/collect-6.2.2)
    ...
    local rl
    rl=$(w623_ratelimited_by_rule "$s" "$e")
    echo "$rl" | sed 's/^/    /'
    ...
    local vol sum pct
    vol=$(w623_volume_delta "$s" "$e")
    sum=$(w623_volume_by_rule "$s" "$e" | awk '{s+=$2} END{printf "%d", s+0}')
    w623_volume_by_rule "$s" "$e" | head -8 | sed 's/^/    /'
    ...
    echo "=== №248 + решение 1: величина (в) поимённо — критерий 6.2.3.2 ==="
    local v_total v_sum v_top v_second
    v_total=$(w623_value_v_total "$s" "$e")
    v_sum=$(w623_value_v_by_rule "$s" "$e" | awk '{s+=$2} END{printf "%d", s+0}')
    w623_value_v_by_rule "$s" "$e" | head -8 | sed 's/^/    /'
    v_top=$(w623_value_v_by_rule "$s" "$e" | awk 'NR==1{print $1}')
    v_second=$(w623_value_v_by_rule "$s" "$e" | awk 'NR==2{print $1}')
    ...
    if [ -n "$want_v" ]; then
        ...
        top_line=$(w623_value_v_by_rule "$s" "$e" | awk -v r="$want_top" '$1==r{print $2}')
        second_line=$(w623_value_v_by_rule "$s" "$e" | awk -v r="$want_second" '$1==r{print $2}')
        ...
        local beacon_v
        beacon_v=$(w623_value_v_by_rule "$s" "$e" | awk '$1=="c2_periodic_beacon_pattern"{print $2}')
    ...
    if [ "$v_total" -le 0 ]; then
        ...
        echo "  покрытие разбивки (в): $pct% (критерий 6.2.3.2 требует ≥ 95%)"
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    case "${1:-}" in
        --self-test) shift; w623_self_test "${1:-}" ;;
