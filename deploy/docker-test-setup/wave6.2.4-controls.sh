...
if [ "${_w624_login_in_win:-0}" -gt 0 ]; then
    echo "  ВНИМАНИЕ: внутрь окна попал ИНТЕРАКТИВНЫЙ ВХОД. Родовые comm (sh/bash/awk/grep/date/pgrep) ниже принадлежат его цепочке MOTD, а не работе измерителя — окно испорчено посторонним подключением (п.2/п.5 гигиены), и величина 6.2.4.1 этого прогона завышена на его вклад"
fi
if [ "${_w624_instr_n:-0}" -eq 0 ] && [ "${_w624_artpath:-0}" -eq 0 ]; then
    pass "6.2.4.3 ДОСТИГНУТО: измеритель внутри окна не работал — ни одного его алерта, ни одного алерта на пути его артефактов (открытый вопрос 16 закрыт живым прогоном)"
else
    die "6.2.4.3 ПРОВАЛЕН: внутри окна ${_w624_instr_n:-0} алертов от comm измерителя и ${_w624_artpath:-0} на пути его артефактов. Это ЧАСТЬ измеренной величины 6.2.4.1, а не поправка к её чтению: контроль обязан не работать внутри окна вовсе (находка №242). Если ненулевая половина — путь артефактов, починка не в порядке операций, а в переносе снимков ВНЕ поддерева, за которым следит drift_new_file_dir_sensitive (открытый вопрос 16)"
fi
