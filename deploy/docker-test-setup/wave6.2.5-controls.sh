...
if [ "$W625_INSTRUMENTED" -eq 1 ] && [ -s "$W625_ART/metrics-window-start.txt" ] && [ -s "$W625_ART/metrics-window-end.txt" ]; then
    ...
    elif [ "$_w625_narrow_bg" -gt 0 ]; then
        die "6.2.3.13 ПРОВАЛЕН (половина «фон»): $_w625_narrow_bg алертов от двух сужённых правил за тихое окно — узкий op=write фильтр не убрал рутинное чтение PAM демоном, сужение №248 не сработало по существу"
    elif [ "${_w625_pos_hit:-0}" -eq 0 ]; then
        die "6.2.3.13 ПРОВАЛЕН (половина «цель»): подделка носила имя sshd/cron и подтверждённо ЗАПИСАЛА в PAM (${_w625_pos_bytes}Б), но ни sigma_failed_login_syscall_daemon, ни rootkit_pam_module_added_daemon не сработали — сужение выродилось в немоту, а не в фильтр по op"
    else
...
_w625_h2_hits=$(_w625_alerts | jq --arg ids "$W625_TWIN_PARENT_RULES" --argjson t "$_w625_t45_start" \
    '[.[]|select((.rule_id as $r|($ids|split(" "))|index($r)) and ((.timestamp|sub("\\.[0-9]+Z$";"Z")|fromdateiso8601? // 0) >= $t) and (.comm=="w625shadow"))]|length' 2>/dev/null)
echo "  (2) позитивная: обычный процесс (comm=w625shadow) прочитал ${_w625_h2_bytes}Б /etc/shadow → алертов родительских правил: ${_w625_h2_hits:-0}"
...
echo "  (3) на подделку: копия /bin/cat с именем cron прочитала ${_w625_h3_bytes}Б /etc/shadow → алертов двойников с comm=cron: ${_w625_h3_hits:-0} (comm алертов периода: ${_w625_h3_comm:-нет})"
...
# ---- ВЕРДИКТ 6.2.4.5: все четыре половины ----
if [ "${_w625_h2_bytes:-0}" -lt 1 ] || [ "${_w625_h3_bytes:-0}" -lt 1 ]; then
    die "6.2.4.5 НЕИЗМЕРИМ: сторож результата не подтвердил чтение (позитивная ${_w625_h2_bytes:-0}Б, подделка ${_w625_h3_bytes:-0}Б) — подача не состоялась, любой ноль ниже приборный (память positive-control-needs-result-sentinel)"
elif [ "$_w625_h1_exc_ok" -ne 1 ]; then
    die "6.2.4.5 ПРОВАЛЕН (половина 1): тишина двойников в окне НЕ подтверждена ненулевым rule_exceptions_total по каждому из четырёх — правило выключено, а не сужено (см. подпункт 6.2.5.1 (1)/(2) выше)"
    ...
    die "6.2.4.5 ПРОВАЛЕН (половина 1): четыре двойника дали ${_w625_h1_alerts} алертов ВНУТРИ тихого окна при работающих исключениях — часть фона ноды исключение не покрывает (поимённо по comm см. 6.2.5.15 выше). Это остаток гейта, а не погрешность"
elif [ "${_w625_h2_hits:-0}" -lt 1 ]; then
    die "6.2.4.5 ПРОВАЛЕН (половина 2): обычный процесс прочитал /etc/shadow (${_w625_h2_bytes}Б), а родительские правила ($W625_TWIN_PARENT_RULES) не поднялись — сужение №253 выродилось в немоту всего класса, а не в исключение фона ноды"
elif [ "${_w625_h3_hits:-0}" -lt 1 ]; then
    die "6.2.4.5 ПРОВАЛЕН (половина 3): подделка носила ИМЯ демона (comm=cron) при чужом образе ($W625_ART/cron) и прочитала ${_w625_h3_bytes}Б /etc/shadow, а двойники не поднялись — исключение идёт за ИМЕНЕМ, а не за образом (немой обход, память comm-not-in-is-a-mute-bypass)"
elif [ "$_w625_cron_lineage_d" -lt 1 ]; then
    die "6.2.4.5 НЕИЗМЕРИМ (половина 4, №261): за ${_w625_cron_wait}s с заданием в /etc/cron.d приращение rule_exceptions_total{verified-daemon-lineage} = 0. Либо ось item 1 не задеплоена, либо cron не форкнулся — в обоих случаях ноль алертов ниже был бы приборным, а не доказательством, что гонка вылечена"
elif [ "${_w625_h4_alerts:-0}" -gt 0 ]; then
    die "6.2.4.5 ПРОВАЛЕН (половина 4, №261): штатный короткоживущий потомок cron поднял ${_w625_h4_alerts} алертов двойников при работающей оси родословной (+$_w625_cron_lineage_d применений) — гонка readlink НЕ вылечена, и остаток гейта (30 алертов/окно в архиве 6.2.4) остаётся на месте"
else
    pass "6.2.4.5 ДОСТИГНУТО: все четыре половины — (1) двойники молчат в окне при ненулевом исключении на каждом; (2) обычный процесс поднимает родительские правила (${_w625_h2_hits}); (3) подделка имени демона поднимает двойника (${_w625_h3_hits}); (4) штатный короткоживущий потомок cron закрыт осью родословной (+$_w625_cron_lineage_d применений, 0 алертов) — ось идёт за образом и за родителем, но не за именем"
fi
