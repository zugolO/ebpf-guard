...
# по которому синтетический сторож №267 считает через w626_metric_sum_file).
#
...
# ---------------------------------------------------------------------------
_W626_JQ_TS='def w626ts: (.timestamp | capture("^(?<i>[^.]+)(\\.(?<f>[0-9]+))?Z$")) as $c
...
w626_tree_price_for_pid() { # $1=alerts.json $2=t0 $3=t1 $4=pid → число алертов окна под деревом этого pid
    [ -s "${1:-}" ] || { printf '0'; return 0; }
    jq --argjson t0 "${2:-0}" --argjson t1 "${3:-0}" --argjson p "${4:-0}" \
        "$_W626_JQ_TS"' [.[] | select((w626ts >= $t0) and (w626ts <= $t1)) | select(rpid == $p)] | length' \
...
w626_force_mechanism_is_winner() { # $1=pid победителя $2=ExecMainPID механизма
    local w="${1:-0}" m="${2:-0}"
...
# ---------------------------------------------------------------------------
w626_c21_why() { # $1=c1_ok $2=c2_ok $3=цена $4=корень $5=6.2.6.1 ok $6=6.2.9.F.7 ok $7=ложных вне окна подов $8=ложных по дереву $9=корни лжи ${10}=цена снята (price_ok)
    local c1="${1:-0}" c2="${2:-0}" price="${3:-?}" root="${4:-?}" gate="${5:-0}" f7="${6:-0}"
    local inc_out="${7:-?}" inc_tree="${8:-?}" names="${9:-?}" price_ok="${10:-}" why=""
    if [ "$c1" -ne 1 ]; then
        ...
        # ФАКТ снятия цены (_w626_c1_price_ok: предикат «своё» действителен И
        ...
        why="условие 1 НЕ ВЗЯТО: величина куста НЕ ИЗМЕРИМА в единице «алертов на одно событие ноды» — ${price_txt}, 6.2.6.1=$([ "$gate" -eq 1 ] && echo OK || echo FAIL), 6.2.9.F.7=$([ "$f7" -eq 1 ] && echo OK || echo FAIL)"
    fi
    if [ "$c2" -ne 1 ]; then
...
w626_self_test() { # $1=каталог архива (…/collect-6.2.2-run1 или …/collect-6.2.2)
    ...
    why_c1=$(w626_c21_why 0 1 0 "(не определён)" 0 0 0 0 нет 0)
    why_c2=$(w626_c21_why 1 0 17 "50-motd-news (pid 16764, одна секунда)" 1 1 1 0 50-motd-news 1)
    why_both=$(w626_c21_why 0 0 0 "(не определён)" 0 1 2 1 "containerd-shim k3s-server" 0)
    ...
    local why_price
    why_price=$(w626_c21_why 0 1 0 "(нет ни одного события ноды с деревом)" 1 1 0 0 нет 0)
    ...
    local why_legacy
    why_legacy=$(w626_c21_why 0 1 0 "(не определён)" 1 1 0 0 нет)
    case "$why_legacy" in
    ...
    if [ -s "$awin" ] && [ -s "$wepoch" ]; then
        ...
        st_top=$(w626_window_top_tree "$awin" "${st_t0:-0}" "${st_t1:-0}" 0)
        ...
        if [ "$base" = "collect-6.2.9.F.1" ]; then
            st_mech=16763
            local st_mech_price
            st_mech_price=$(w626_tree_price_for_pid "$awin" "${st_t0:-0}" "${st_t1:-0}" "$st_mech")
            ...
            if w626_force_mechanism_is_winner "$st_top_pid" "$st_mech"; then
            ...
            if w626_force_mechanism_is_winner "$st_top_pid" 16764; then
            ...
            if w626_force_mechanism_is_winner 0 0; then
    ...
    case "$base" in
        collect-6.2.2|collect-6.2.2-run1|collect-6.2.4|collect-6.2.5)
            if printf '%s' "$axes_arch" | grep -q 'field=НЕТ_ЛЕЙБЛА'; then
                ...
                echo "  ПРОВАЛ: архив «» снят ДО item 3 и обязан нести метрику БЕЗ лейбла field, а разбор этого не сказал: $axes_arch"; rc=1
            fi ;;
        *)
            if printf '%s' "$axes_arch" | grep -q 'field=НЕТ_ЛЕЙБЛА'; then
                echo "  ПРОВАЛ: архив «» снят ПОСЛЕ item 3, но лейбла field в метрике нет — item 3 не задеплоен либо метрика переименована"; rc=1
            else
                echo "  OK: архив «» снят ПОСЛЕ item 3 — оси разобраны поимённо, НЕТ_ЛЕЙБЛА не встречается"
            fi ;;
    esac
...
EOF
    ...
    {
        echo "$(w626_label_key '6.2.6.1 ДОСТИГНУТО: цена ноды 54 алертов/ч ≤ 100/ч') OK"
        echo "$(w626_label_key '6.2.9.F.2 НЕИЗМЕРИМ (№295): оценка цены снятия величины сравнима с запасом гейта — вердикт 6.2.6.1 на этом снимке не выносится') FAIL"
        echo "$(w626_label_key '6.2.9.F.3 ИЗМЕРЕНО (№293): доля атрибутирована по дереву предков') OK"
    } > "$reg2"
    sed 's/^/    /' "$reg2"
    if w626_label_taken 6.2.6.1 "$reg2" \
       && ! w626_label_taken 6.2.9.F.2 "$reg2" \
       && w626_label_taken 6.2.9.F.3 "$reg2"; then
