...
PYEOF
        ...
        if ! kill -0 "$_631u1_listener_pid" 2>/dev/null; then
            ...
            # ИМЯ ЗОНДА ОБЯЗАНО БЫТЬ ДОБРОКАЧЕСТВЕННЫМ ПО ШКАЛЕ DGA (№399).
            # Прежнее «w63u1-ipv6-probe-<20 случайных>» набирало по биграммной
            # модели 0.552 — выше порога 0.55 — и удовлетворяло структурной
            # половине is_dga_domain. Событие уезжало в Rego, dga_domain
            # матчился, и обогащение ПЕРЕИМЕНОВЫВАЛО алерт: rule_id становился
            # dga_domain, а счётчик базового dns_any_query не двигался вовсе.
            # Метка выносила ПРОВАЛ при видимом событии (Δ=2) и поднятом
            # алерте — просто под другим именем.
            # Новое имя: первая метка несёт словарное «dns», поэтому
            # структурная половина ЛОЖНА при любой оценке, вторая метка короче
            # ngramMinLabelLen и моделью не судится. Измерено: score 0.470,
            # префильтр НЕ форвардит вовсе — переименовать алерт нечему.
            # Пришпилено офлайн: TestWave6_3u_StandProbeFixturesStayAttributable.
            _631u1_domain="w63u1-dns-probe.p$(head -c 4 /dev/urandom 2>/dev/null | od -An -tx1 | tr -d ' \n' | head -c 4).invalid"
            # `+notcp` ОБЯЗАТЕЛЕН вместе с ANY (№398, измерено на стенде,
            # DiG 9.18.39): начиная с 9.16 dig шлёт ANY-запрос ПО TCP, а
            # слушатель контроля — UDP-сокет, ответа нет, rc=9 «no reply», и
            # метка честно печатает НЕИЗМЕРИМ вместо измерения. Проверено
            # прямо, тремя вызовами подряд на одном слушателе: `x1.invalid`
            # rc=0, `x2.invalid ANY` rc=9, `+notcp x3.invalid ANY` rc=0.
            # По существу флаг тоже верен: предмет метки — UDP/IPv6, а TCP-DNS
            # это отдельный путь коллектора (№357) со своей меткой.
            dig +short +time=2 +tries=1 +notcp -6 @::1 -p 53 "$_631u1_domain" ANY >/dev/null 2>&1
            _631u1_rc=$?
            sleep "${W3_593C_SLEEP:-15}"
            ...
            if [ ! -s "$_631u1_stub_dir/metrics-before.txt" ] || [ ! -s "$_631u1_stub_dir/metrics-after.txt" ]; then
                ...
                # ([[positive-control-needs-result-sentinel]]).
                die "6.3u.1 НЕИЗМЕРИМ: класс=IPv6-запрос не ушёл (dig +notcp -6 @::1 … ANY rc=$_631u1_rc; rc=9 = ответа нет — слушатель мёртв либо dig ушёл в TCP, см. №398) — величины ниже приборные, а не вердикт о детекте"
