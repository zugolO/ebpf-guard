...
// -----------------------------------------------------------------------
// TLS uprobe collector (Issue #35)
// -----------------------------------------------------------------------

// TLSObjects contains all eBPF objects for TLS plaintext capture via uprobes.
// Programs attach to SSL_write / SSL_read in libssl.so at runtime.
...
func (o *TLSObjects) Close() error {
	for _, p := range []*ebpf.Program{o.TraceSslWrite, o.TraceSslReadEntry, o.TraceSslReadRet} {
		if p != nil {
			p.Close()
		}
	}
	for _, m := range []*ebpf.Map{o.TlsEvents, o.SslReadContexts} {
		if m != nil {
			m.Close()
		}
	}
...
// LoadTLSObjects loads TLS uprobe eBPF objects from embedded bytecode.
// In production this is generated by bpf2go from bpf/tls_uprobe.bpf.c.
func LoadTLSObjects(obj *TLSObjects, opts *ebpf.CollectionOptions) error {
	return fmt.Errorf("bpf2go generated code not available: run 'make generate' with clang installed")
}
