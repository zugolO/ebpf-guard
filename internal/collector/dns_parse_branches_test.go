...
func TestParseDNSWireMessage_BadQName(t *testing.T) {
	...
	assert.Equal(t, dnsDecodeReasonBadQName, reason, "a malformed QNAME must fail the whole message")
}
...
func TestDecodeDNSName_TruncatedPointer(t *testing.T) {
	_, _, reason := decodeDNSName([]byte{0xC0}, 0)
	assert.Equal(t, dnsDecodeReasonBadQName, reason, "a one-byte compression pointer must be rejected")
}
...
func TestDecodeDNSName_PointerOutOfRange(t *testing.T) {
	...
	assert.Equal(t, dnsDecodeReasonBadQName, reason, "a pointer past the end of the message must be rejected")
}
