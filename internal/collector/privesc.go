...
func (c *PrivescCollector) readLoop(ctx context.Context, out chan<- types.Event) {
	for {
		...
		sendEvent(ctx, out, *event, c.strategy, func() {
			exporter.RecordEventDrop("privesc", "ringbuf_to_router", defaultEventPriority(event.Type))
...
func (c *PrivescCollector) RingbufFullMap() *ebpf.Map {
	if c.objs == nil {
		return nil
	}
...
func (c *PrivescCollector) EmittedMap() *ebpf.Map {
	if c.objs == nil {
		return nil
	}
